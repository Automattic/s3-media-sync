...
BASH
    ...
    if (\JSON_ERROR_NONE !== \json_last_error()) {
        ...
        throw new \RuntimeException(\sprintf('Could not parse the request contents: "%d: %s"', \json_last_error(), \json_last_error_msg()));
    }
    if (\false === isset($contents->tag_name) || \false === \is_string($contents->tag_name)) {
        throw new \RuntimeException(\sprintf('No tag name could be found in: %s', $lastReleaseEndpointContents), 100);
    }
    if ('' !== ($lastRelease = \trim($contents->tag_name))) {
        return $lastRelease;
    }
