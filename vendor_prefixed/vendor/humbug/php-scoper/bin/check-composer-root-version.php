...
try {
    $composerRootVersion = \WPCOM_VIP\get_composer_root_version(\WPCOM_VIP\get_last_tag_name());
...
if ($composerRootVersion !== $currentRootVersion) {
    \file_put_contents('php://stderr', \sprintf('Expected the COMPOSER_ROOT_VERSION to be "%s" but got "%s" instead.' . \PHP_EOL, $composerRootVersion, $currentRootVersion));
    exit(1);
}
