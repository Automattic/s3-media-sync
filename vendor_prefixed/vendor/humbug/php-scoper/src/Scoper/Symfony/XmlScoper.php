 ...
 */
namespace WPCOM_VIP\Humbug\PhpScoper\Scoper\Symfony;
 ...
 */
final class XmlScoper implements \WPCOM_VIP\Humbug\PhpScoper\Scoper
{
    ...
    private function scopeClasses(string $contents, string $prefix, \WPCOM_VIP\Humbug\PhpScoper\Whitelist $whitelist) : string
    {
    ...
    private function scopeNamespaces(string $contents, string $prefix, \WPCOM_VIP\Humbug\PhpScoper\Whitelist $whitelist) : string
    {
        if (1 > \preg_match_all('/<prototype.*\\snamespace="(?:(?<namespace>(?:[^\\\\]+(?<separator>\\\\(?:\\\\)?))))"/', $contents, $matches)) {
            return $contents;
        }
        return $this->replaceClasses(\array_filter($matches['namespace']), \array_filter($matches['separator']), $prefix, $contents, $whitelist);
    }
    /**
     * @param string[] $classes
     * @param string[] $separators
     */
    private function replaceClasses(array $classes, array $separators, string $prefix, string $contents, \WPCOM_VIP\Humbug\PhpScoper\Whitelist $whitelist) : string
    {
        if ([] === $classes) {
            return $contents;
        }
        ...
        foreach ($classes as $index => $class) {
            ...
            if ($whitelist->isSymbolWhitelisted($class) || $whitelist->isGlobalWhitelistedClass($class)) {
                $whitelist->recordWhitelistedClass(new \WPCOM_VIP\PhpParser\Node\Name\FullyQualified($class), new \WPCOM_VIP\PhpParser\Node\Name\FullyQualified($prefixedClass));
            }
        }
