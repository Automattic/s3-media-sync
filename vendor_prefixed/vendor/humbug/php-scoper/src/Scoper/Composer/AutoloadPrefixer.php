...
final class AutoloadPrefixer
{
     ...
     */
    public static function prefixPackageAutoloadStatements(\stdClass $contents, string $prefix, \WPCOM_VIP\Humbug\PhpScoper\Whitelist $whitelist) : \stdClass
    {
    ...
    private static function prefixAutoloadStatements(\stdClass $autoload, string $prefix, \WPCOM_VIP\Humbug\PhpScoper\Whitelist $whitelist) : \stdClass
    {
    ...
    private static function prefixAutoload(array $autoload, string $prefix, \WPCOM_VIP\Humbug\PhpScoper\Whitelist $whitelist) : array
    {
        $loader = [];
        foreach ($autoload as $namespace => $paths) {
            $newNamespace = $whitelist->isWhitelistedNamespace($namespace) ? $namespace : \sprintf('%s\\%s', $prefix, $namespace);
    ...
    private static function prefixLaravelProviders(array $providers, string $prefix, \WPCOM_VIP\Humbug\PhpScoper\Whitelist $whitelist) : array
    {
        return \array_map(static function (string $provider) use($prefix, $whitelist) : string {
            return $whitelist->isWhitelistedNamespace($provider) ? $provider : \sprintf('%s\\%s', $prefix, $provider);
        }, $providers);
    }
}
