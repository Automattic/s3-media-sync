...
final class JsonFileScoper implements \WPCOM_VIP\Humbug\PhpScoper\Scoper
{
    ...
    public function scope(string $filePath, string $contents, string $prefix, array $patchers, \WPCOM_VIP\Humbug\PhpScoper\Whitelist $whitelist) : string
    {
        ...
        if (\false === $decodedJson instanceof \stdClass) {
            throw new \LogicException(\sprintf('Expected the decoded JSON to be an stdClass instance, got "%s" instead', \gettype($decodedJson)));
        }
        $decodedJson = \WPCOM_VIP\Humbug\PhpScoper\Scoper\Composer\AutoloadPrefixer::prefixPackageAutoloadStatements($decodedJson, $prefix, $whitelist);
