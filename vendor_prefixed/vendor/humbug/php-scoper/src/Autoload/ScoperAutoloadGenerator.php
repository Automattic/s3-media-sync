 ...
 */
namespace WPCOM_VIP\Humbug\PhpScoper\Autoload;
...
final class ScoperAutoloadGenerator
{
    private $whitelist;
    private $eol;
    public function __construct(\WPCOM_VIP\Humbug\PhpScoper\Whitelist $whitelist)
    {
    ...
    public function dump() : string
    {
        ...
        $statements = \implode($this->eol, $this->createClassAliasStatements($this->whitelist->getRecordedWhitelistedClasses(), $hasNamespacedFunctions)) . $this->eol . $this->eol;
        $statements .= \implode($this->eol, $this->createFunctionAliasStatements($whitelistedFunctions, $hasNamespacedFunctions));
...
PHP;
    ...
    {
        $statements = \array_map(static function (array $pair) : string {
            /**
             * @var string
...
, $originalClass, $prefixedClass);
        ...
        if ($hasNamespacedFunctions) {
            $eol = $this->eol;
            $statements = \array_map(static function (string $statement) use($eol) : string {
                return \implode($eol, \array_map(static function (string $statement) : string {
...
EOF
    ...
    {
        $statements = \array_map(static function (array $node) use($hasNamespacedFunctions) : string {
            $original = new \WPCOM_VIP\PhpParser\Node\Name\FullyQualified($node[0]);
            $alias = new \WPCOM_VIP\PhpParser\Node\Name\FullyQualified($node[1]);
            if ($hasNamespacedFunctions) {
...
PHP
, null === $namespace ? '' : $namespace->toString() . ' ', $original->toString(), $functionName, '__autoload' === $functionName ? '$className' : '', $alias->toString());
            }
            return \sprintf(<<<'PHP'
...
PHP
, $original, '__autoload' === (string) $original ? '$className' : '', $alias);
