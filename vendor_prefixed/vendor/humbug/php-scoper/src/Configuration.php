...
use WPCOM_VIP\Humbug\PhpScoper\Patcher\SymfonyPatcher;
...
final class Configuration
{
    ...
    private const KEYWORDS = [self::PREFIX_KEYWORD, self::WHITELISTED_FILES_KEYWORD, self::FINDER_KEYWORD, self::PATCHERS_KEYWORD, self::WHITELIST_KEYWORD, self::WHITELIST_GLOBAL_CONSTANTS_KEYWORD, self::WHITELIST_GLOBAL_CLASSES_KEYWORD, self::WHITELIST_GLOBAL_FUNCTIONS_KEYWORD];
    private $path;
    private $prefix;
    ...
    public static function load(string $path = null, array $paths = []) : self
    {
        if (null === $path) {
            ...
            if (\false === (new \WPCOM_VIP\Symfony\Component\Filesystem\Filesystem())->isAbsolutePath($path)) {
        ...
        \array_unshift($patchers, new \WPCOM_VIP\Humbug\PhpScoper\Patcher\SymfonyPatcher());
     ...
     */
    private function __construct(?string $path, ?string $prefix, array $filesWithContents, array $patchers, \WPCOM_VIP\Humbug\PhpScoper\Whitelist $whitelist, array $whitelistedFiles)
    {
        $this->path = $path;
        $this->prefix = $prefix;
    ...
    public function withPaths(array $paths) : self
    {
        ...
        return new self($this->path, $this->prefix, \array_merge($this->filesWithContents, $filesWithContents), $this->patchers, $this->whitelist, $this->whitelistedFiles);
    }
    public function withPrefix(?string $prefix) : self
    {
        $prefix = self::retrievePrefix([self::PREFIX_KEYWORD => $prefix]);
        return new self($this->path, $prefix, $this->filesWithContents, $this->patchers, $this->whitelist, $this->whitelistedFiles);
    }
    ...
    public function getWhitelist() : \WPCOM_VIP\Humbug\PhpScoper\Whitelist
    {
    ...
    {
        \array_map([self::class, 'validateConfigKey'], \array_keys($config));
    }
    private static function validateConfigKey(string $key) : void
    {
        if (\false === \in_array($key, self::KEYWORDS, \true)) {
    ...
    private static function retrieveWhitelist(array $config) : \WPCOM_VIP\Humbug\PhpScoper\Whitelist
    {
        if (\false === \array_key_exists(self::WHITELIST_KEYWORD, $config)) {
            ...
            if (\false === \is_array($whitelist)) {
                throw new \InvalidArgumentException(\sprintf('Expected whitelist to be an array of strings, found "%s" instead.', \gettype($whitelist)));
            }
            foreach ($whitelist as $index => $className) {
                if (\is_string($className)) {
                    continue;
                }
                throw new \InvalidArgumentException(\sprintf('Expected whitelist to be an array of string, the "%d" element is not.', $index));
            }
        }
        if (\false === \array_key_exists(self::WHITELIST_GLOBAL_CONSTANTS_KEYWORD, $config)) {
            $whitelistGlobalConstants = \true;
        } else {
            $whitelistGlobalConstants = $config[self::WHITELIST_GLOBAL_CONSTANTS_KEYWORD];
            if (\false === \is_bool($whitelistGlobalConstants)) {
                throw new \InvalidArgumentException(\sprintf('Expected %s to be a boolean, found "%s" instead.', self::WHITELIST_GLOBAL_CONSTANTS_KEYWORD, \gettype($whitelistGlobalConstants)));
            }
        }
        if (\false === \array_key_exists(self::WHITELIST_GLOBAL_CLASSES_KEYWORD, $config)) {
            $whitelistGlobalClasses = \true;
        } else {
            $whitelistGlobalClasses = $config[self::WHITELIST_GLOBAL_CLASSES_KEYWORD];
            if (\false === \is_bool($whitelistGlobalClasses)) {
                throw new \InvalidArgumentException(\sprintf('Expected %s to be a boolean, found "%s" instead.', self::WHITELIST_GLOBAL_CLASSES_KEYWORD, \gettype($whitelistGlobalClasses)));
            }
        }
        if (\false === \array_key_exists(self::WHITELIST_GLOBAL_FUNCTIONS_KEYWORD, $config)) {
            $whitelistGlobalFunctions = \true;
        } else {
            $whitelistGlobalFunctions = $config[self::WHITELIST_GLOBAL_FUNCTIONS_KEYWORD];
            if (\false === \is_bool($whitelistGlobalFunctions)) {
                throw new \InvalidArgumentException(\sprintf('Expected %s to be a boolean, found "%s" instead.', self::WHITELIST_GLOBAL_FUNCTIONS_KEYWORD, \gettype($whitelistGlobalFunctions)));
            }
        }
        return \WPCOM_VIP\Humbug\PhpScoper\Whitelist::create($whitelistGlobalConstants, $whitelistGlobalClasses, $whitelistGlobalFunctions, ...$whitelist);
    }
    /**
    ...
    private static function retrieveWhitelistedFiles(string $dirPath, array $config) : array
    {
        ...
        if (\false === \is_array($whitelistedFiles)) {
            throw new \InvalidArgumentException(\sprintf('Expected whitelisted files to be an array of strings, found "%s" instead.', \gettype($whitelistedFiles)));
        }
        foreach ($whitelistedFiles as $index => $file) {
            if (\false === \is_string($file)) {
                throw new \InvalidArgumentException(\sprintf('Expected whitelisted files to be an array of string, the "%d" element is not.', $index));
            }
            if (\false === (new \WPCOM_VIP\Symfony\Component\Filesystem\Filesystem())->isAbsolutePath($file)) {
    ...
    private static function retrieveFinders(array $config) : array
    {
        ...
        if (\false === \is_array($finders)) {
            throw new \InvalidArgumentException(\sprintf('Expected finders to be an array of "%s", found "%s" instead.', \WPCOM_VIP\Symfony\Component\Finder\Finder::class, \gettype($finders)));
        }
        foreach ($finders as $index => $finder) {
            if ($finder instanceof \WPCOM_VIP\Symfony\Component\Finder\Finder) {
                continue;
            }
            throw new \InvalidArgumentException(\sprintf('Expected finders to be an array of "%s", the "%d" element is not.', \WPCOM_VIP\Symfony\Component\Finder\Finder::class, $index));
        }
        return $finders;
    }
    /**
    ...
    private static function retrieveFilesFromPaths(array $paths) : iterable
    {
        ...
        $finder = new \WPCOM_VIP\Symfony\Component\Finder\Finder();
        $finder->files()->in($pathsToSearch)->append($filesToAppend)->filter(static function (\SplFileInfo $fileInfo) : ?bool {
            if ($fileInfo->isLink()) {
                return \false;
            }
            return null;
    ...
    {
        return \array_reduce(\iterator_to_array($files, \false), static function (array $files, \SplFileInfo $fileInfo) : array {
            $file = $fileInfo->getRealPath();
            if (\false === $file) {
                throw new \RuntimeException(\sprintf('Could not find the file "%s".', (string) $fileInfo));
            }
            if (\false === \is_readable($file)) {
                throw new \RuntimeException(\sprintf('Could not read the file "%s".', $file));
            }
