...
final class Whitelist implements \Countable
{
    ...
    public static function create(bool $whitelistGlobalConstants, bool $whitelistGlobalClasses, bool $whitelistGlobalFunctions, string ...$elements) : self
    {
        $symbols = [];
        $constants = [];
        $namespaces = [];
        ...
        foreach ($elements as $element) {
            ...
            if ('\\*' === \substr($element, -2)) {
                ...
                $patterns[] = \sprintf('/^%s$/u', \str_replace('\\', '\\\\', \str_replace('*', '.*', $element)));
            } else {
                $symbols[] = \strtolower($element);
                $constants[] = self::lowerConstantName($element);
            }
        }
        return new self($whitelistGlobalConstants, $whitelistGlobalClasses, $whitelistGlobalFunctions, \array_unique($original), \array_flip($symbols), \array_flip($constants), \array_unique($patterns), \array_unique($namespaces));
    }
     ...
     */
    private function __construct(bool $whitelistGlobalConstants, bool $whitelistGlobalClasses, bool $whitelistGlobalFunctions, array $original, array $symbols, array $constants, array $patterns, array $namespaces)
    {
    ...
    public function recordWhitelistedFunction(\WPCOM_VIP\PhpParser\Node\Name\FullyQualified $original, \WPCOM_VIP\PhpParser\Node\Name\FullyQualified $alias) : void
    {
    ...
    public function recordWhitelistedClass(\WPCOM_VIP\PhpParser\Node\Name\FullyQualified $original, \WPCOM_VIP\PhpParser\Node\Name\FullyQualified $alias) : void
    {
    ...
    {
        return \array_filter($this->original, static function (string $name) : bool {
