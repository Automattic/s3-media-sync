...
$jetBrainStubs = (static function () : array {
    ...
    foreach (new \DirectoryIterator(__DIR__ . '/vendor/jetbrains/phpstorm-stubs') as $directoryInfo) {
        ...
        if (\in_array($directoryInfo->getBasename(), ['tests', 'meta'], \true)) {
            continue;
        }
...
return ['whitelist' => [\WPCOM_VIP\Isolated\Symfony\Component\Finder\Finder::class], 'files-whitelist' => $jetBrainStubs, 'patchers' => [
    //
    ...
    static function (string $filePath, string $prefix, string $contents) : string {
        ...
        return \str_replace([$prefix . '\\\\', $prefix . '\\', 'namespace JetBrains\\PHPStormStub;'], ['', '', \sprintf('namespace %s\\JetBrains\\PHPStormStub;', $prefix)], $contents);
    },
    //
    ...
    static function (string $filePath, string $prefix, string $contents) : string {
        ...
        return \sprintf('%s%s', \substr($contents, 0, $prefixedClassPosition), \substr($originalContents, $classPosition));
    },
    static function (string $filePath, string $prefix, string $contents) : string {
        if ('bin/php-scoper' !== $filePath) {
            return $contents;
        }
        return \str_replace('\\' . $prefix . '\\Isolated\\Symfony\\Component\\Finder\\Finder::class', '\\Isolated\\Symfony\\Component\\Finder\\Finder::class', $contents);
    },
]];
