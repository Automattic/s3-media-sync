...
class NodeTraverser implements \WPCOM_VIP\PhpParser\NodeTraverserInterface
{
    /**
     ...
     */
    public function addVisitor(\WPCOM_VIP\PhpParser\NodeVisitor $visitor)
    {
     ...
     */
    public function removeVisitor(\WPCOM_VIP\PhpParser\NodeVisitor $visitor)
    {
     ...
     */
    protected function traverseNode(\WPCOM_VIP\PhpParser\Node $node) : \WPCOM_VIP\PhpParser\Node
    {
        foreach ($node->getSubNodeNames() as $name) {
            ...
            } elseif ($subNode instanceof \WPCOM_VIP\PhpParser\Node) {
                $traverseChildren = \true;
                $breakVisitorIndex = null;
                foreach ($this->visitors as $visitorIndex => $visitor) {
                    $return = $visitor->enterNode($subNode);
                    if (null !== $return) {
                        if ($return instanceof \WPCOM_VIP\PhpParser\Node) {
    ...
    {
        if ($old instanceof \WPCOM_VIP\PhpParser\Node\Stmt && $new instanceof \WPCOM_VIP\PhpParser\Node\Expr) {
        ...
        if ($old instanceof \WPCOM_VIP\PhpParser\Node\Expr && $new instanceof \WPCOM_VIP\PhpParser\Node\Stmt) {
