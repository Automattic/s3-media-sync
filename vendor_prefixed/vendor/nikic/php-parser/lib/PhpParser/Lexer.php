...
use WPCOM_VIP\PhpParser\Parser\Tokens;
class Lexer
{
    protected $code;
    protected $tokens;
     ...
     */
    public function startLexing(string $code, \WPCOM_VIP\PhpParser\ErrorHandler $errorHandler = null)
    {
    ...
    private function handleInvalidCharacterRange($start, $end, $line, \WPCOM_VIP\PhpParser\ErrorHandler $errorHandler)
    {
        $tokens = [];
        for ($i = $start; $i < $end; $i++) {
            $chr = $this->code[$i];
            ...
            $errorHandler->handleError(new \WPCOM_VIP\PhpParser\Error($errorMsg, ['startLine' => $line, 'endLine' => $line, 'startFilePos' => $i, 'endFilePos' => $i]));
        }
        return $tokens;
    }
    /**
    ...
    protected function postprocessTokens(\WPCOM_VIP\PhpParser\ErrorHandler $errorHandler)
    {
        ...
        if ($filePos !== \strlen($this->code)) {
            if (\substr($this->code, $filePos, 2) === '/*') {
                ...
                $errorHandler->handleError(new \WPCOM_VIP\PhpParser\Error('Unterminated comment', ['startLine' => $line, 'endLine' => $line + \substr_count($comment, "\n"), 'startFilePos' => $filePos, 'endFilePos' => $filePos + \strlen($comment)]));
        ...
        if (\count($this->tokens) > 0) {
            ...
            if ($this->isUnterminatedComment($lastToken)) {
                $errorHandler->handleError(new \WPCOM_VIP\PhpParser\Error('Unterminated comment', ['startLine' => $line - \substr_count($lastToken[1], "\n"), 'endLine' => $line, 'startFilePos' => $filePos - \strlen($lastToken[1]), 'endFilePos' => $filePos]));
            }
        }
    }
    /**
            ...
            if (\is_string($token)) {
                ...
                if (\T_COMMENT === $token[0] || \T_DOC_COMMENT === $token[0]) {
                    if ($this->attributeCommentsUsed) {
                        $comment = \T_DOC_COMMENT === $token[0] ? new \WPCOM_VIP\PhpParser\Comment\Doc($token[1], $origLine, $origFilePos, $this->pos, $this->line, $this->filePos - 1, $this->pos) : new \WPCOM_VIP\PhpParser\Comment($token[1], $origLine, $origFilePos, $this->pos, $this->line, $this->filePos - 1, $this->pos);
    ...
    public function handleHaltCompiler() : string
    {
        ...
        if (!\preg_match('~^\\s*\\(\\s*\\)\\s*(?:;|\\?>\\r?\\n?)~', $textAfter, $matches)) {
            throw new \WPCOM_VIP\PhpParser\Error('__HALT_COMPILER must be followed by "();"');
        }
    ...
    protected function createTokenMap() : array
    {
        ...
        for ($i = 256; $i < 1000; ++$i) {
            if (\T_DOUBLE_COLON === $i) {
                ...
                $tokenMap[$i] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_PAAMAYIM_NEKUDOTAYIM;
            } elseif (\T_OPEN_TAG_WITH_ECHO === $i) {
                // T_OPEN_TAG_WITH_ECHO with dropped T_OPEN_TAG results in T_ECHO
                $tokenMap[$i] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_ECHO;
                ...
                if ('T_HASHBANG' === $name) {
                    ...
                    $tokenMap[$i] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_INLINE_HTML;
                } elseif (\defined($name = \WPCOM_VIP\PhpParser\Parser\Tokens::class . '::' . $name)) {
        ...
        if (\defined('T_ONUMBER')) {
            $tokenMap[\T_ONUMBER] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_DNUMBER;
        }
        // HHVM also has a separate token for the __COMPILER_HALT_OFFSET__ constant
        if (\defined('T_COMPILER_HALT_OFFSET')) {
            $tokenMap[\T_COMPILER_HALT_OFFSET] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_STRING;
        }
        // Assign tokens for which we define compatibility constants, as token_name() does not know them.
        $tokenMap[\T_FN] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_FN;
        $tokenMap[\T_COALESCE_EQUAL] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_COALESCE_EQUAL;
        $tokenMap[\T_NAME_QUALIFIED] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_NAME_QUALIFIED;
        $tokenMap[\T_NAME_FULLY_QUALIFIED] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_NAME_FULLY_QUALIFIED;
        $tokenMap[\T_NAME_RELATIVE] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_NAME_RELATIVE;
        $tokenMap[\T_MATCH] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_MATCH;
        $tokenMap[\T_NULLSAFE_OBJECT_OPERATOR] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_NULLSAFE_OBJECT_OPERATOR;
        $tokenMap[\T_ATTRIBUTE] = \WPCOM_VIP\PhpParser\Parser\Tokens::T_ATTRIBUTE;
        return $tokenMap;
    }
