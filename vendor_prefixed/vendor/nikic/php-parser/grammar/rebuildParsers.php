...
foreach ($grammarFileToName as $grammarFile => $name) {
    ...
    $grammarCode = \WPCOM_VIP\resolveNodes($grammarCode);
    $grammarCode = \WPCOM_VIP\resolveMacros($grammarCode);
    $grammarCode = \WPCOM_VIP\resolveStackAccess($grammarCode);
    \file_put_contents($tmpGrammarFile, $grammarCode);
    $additionalArgs = $optionDebug ? '-t -v' : '';
    echo "Building {$name} parser.\n";
    $output = \WPCOM_VIP\execCmd("{$kmyacc} {$additionalArgs} -m {$skeletonFile} -p {$name} {$tmpGrammarFile}");
    $resultCode = \file_get_contents($tmpResultFile);
    $resultCode = \WPCOM_VIP\removeTrailingWhitespace($resultCode);
    \WPCOM_VIP\ensureDirExists($resultDir);
    \file_put_contents("{$resultDir}/{$name}.php", $resultCode);
    \unlink($tmpResultFile);
    echo "Building token definition.\n";
    $output = \WPCOM_VIP\execCmd("{$kmyacc} -m {$tokensTemplate} {$tmpGrammarFile}");
...
function resolveNodes($code)
{
    return \preg_replace_callback('~\\b(?<name>[A-Z][a-zA-Z_\\\\]++)\\s*' . \PARAMS . '~', function ($matches) {
        // recurse
        $matches['params'] = \WPCOM_VIP\resolveNodes($matches['params']);
        $params = \WPCOM_VIP\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['params']);
        $paramCode = '';
...
function resolveMacros($code)
{
    return \preg_replace_callback('~\\b(?<!::|->)(?!array\\()(?<name>[a-z][A-Za-z]++)' . \ARGS . '~', function ($matches) {
        // recurse
        $matches['args'] = \WPCOM_VIP\resolveMacros($matches['args']);
        $name = $matches['name'];
        $args = \WPCOM_VIP\magicSplit('(?:' . \PARAMS . '|' . \ARGS . ')(*SKIP)(*FAIL)|,', $matches['args']);
        if ('attributes' === $name) {
            \WPCOM_VIP\assertArgs(0, $args, $name);
            return '$this->startAttributeStack[#1] + $this->endAttributes';
        }
        if ('stackAttributes' === $name) {
            \WPCOM_VIP\assertArgs(1, $args, $name);
        ...
        if ('push' === $name) {
            \WPCOM_VIP\assertArgs(2, $args, $name);
        ...
        if ('parseEncapsed' === $name) {
            \WPCOM_VIP\assertArgs(3, $args, $name);
...
{
    $pieces = \preg_split(\WPCOM_VIP\regex('(?:(?&string)|(?&comment)|(?&code))(*SKIP)(*FAIL)|' . $regex), $string);
