...
class ResultPaginator implements \Iterator
{
     ...
     * @param array              $config
     */
    public function __construct(\WPCOM_VIP\Aws\AwsClientInterface $client, $operation, array $args, array $config)
    {
        $this->client = $client;
    ...
    {
        return \WPCOM_VIP\GuzzleHttp\Promise\coroutine(function () use($handleResult) {
            $nextToken = null;
            do {
                ...
                if ($retVal !== null) {
                    (yield \WPCOM_VIP\GuzzleHttp\Promise\promise_for($retVal));
                }
            } while ($nextToken);
        });
    }
    ...
    public function search($expression)
    {
        ...
        return flatmap($this, function (\WPCOM_VIP\Aws\Result $result) use($expression) {
    ...
    private function determineNextToken(\WPCOM_VIP\Aws\Result $result)
    {
