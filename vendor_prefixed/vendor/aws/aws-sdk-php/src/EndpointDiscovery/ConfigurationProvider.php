...
use WPCOM_VIP\Aws\EndpointDiscovery\Exception\ConfigurationException;
...
class ConfigurationProvider extends \WPCOM_VIP\Aws\AbstractConfigurationProvider implements \WPCOM_VIP\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPCOM_VIP\Aws\EndpointDiscovery\ConfigurationInterface::class;
    protected static $exceptionClass = \WPCOM_VIP\Aws\EndpointDiscovery\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (isset($config['endpoint_discovery']) && $config['endpoint_discovery'] instanceof \WPCOM_VIP\Aws\CacheInterface) {
    ...
    public static function env($cacheLimit = self::DEFAULT_CACHE_LIMIT)
    {
        return function () use($cacheLimit) {
            ...
            if ($enabled !== \false && $enabled !== '') {
                return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\EndpointDiscovery\Configuration($enabled, $cacheLimit));
            }
    ...
    public static function fallback($config = [])
    {
        ...
        return function () use($enabled) {
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\EndpointDiscovery\Configuration($enabled, self::DEFAULT_CACHE_LIMIT));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null, $cacheLimit = self::DEFAULT_CACHE_LIMIT)
    {
        ...
        return function () use($profile, $filename, $cacheLimit) {
            ...
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\EndpointDiscovery\Configuration($data[$profile]['endpoint_discovery_enabled'], $cacheLimit));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof \WPCOM_VIP\Aws\EndpointDiscovery\ConfigurationInterface) {
            return $config;
        } elseif (\is_array($config) && isset($config['enabled'])) {
            if (isset($config['cache_limit'])) {
                return new \WPCOM_VIP\Aws\EndpointDiscovery\Configuration($config['enabled'], $config['cache_limit']);
            }
            return new \WPCOM_VIP\Aws\EndpointDiscovery\Configuration($config['enabled'], self::DEFAULT_CACHE_LIMIT);
        }
