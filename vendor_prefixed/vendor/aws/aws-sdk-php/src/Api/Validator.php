...
class Validator
{
     ...
     */
    public function validate($name, \WPCOM_VIP\Aws\Api\Shape $shape, array $input)
    {
    ...
    private function dispatch(\WPCOM_VIP\Aws\Api\Shape $shape, $value)
    {
    ...
    private function check_structure(\WPCOM_VIP\Aws\Api\StructureShape $shape, $value)
    {
    ...
    private function check_list(\WPCOM_VIP\Aws\Api\ListShape $shape, $value)
    {
        if (!\is_array($value)) {
            $this->addError('must be an array. Found ' . \WPCOM_VIP\Aws\describe_type($value));
            return;
        }
    ...
    private function check_map(\WPCOM_VIP\Aws\Api\MapShape $shape, $value)
    {
        if (!$this->checkAssociativeArray($value)) {
            return;
        }
    ...
    private function check_blob(\WPCOM_VIP\Aws\Api\Shape $shape, $value)
    {
        static $valid = ['string' => \true, 'integer' => \true, 'double' => \true, 'resource' => \true];
        $type = \gettype($value);
        if (!isset($valid[$type])) {
            if ($type != 'object' || !\method_exists($value, '__toString')) {
                $this->addError('must be an fopen resource, a ' . 'GuzzleHttp\\Stream\\StreamInterface object, or something ' . 'that can be cast to a string. Found ' . \WPCOM_VIP\Aws\describe_type($value));
            }
        }
    }
    private function check_numeric(\WPCOM_VIP\Aws\Api\Shape $shape, $value)
    {
        if (!\is_numeric($value)) {
            $this->addError('must be numeric. Found ' . \WPCOM_VIP\Aws\describe_type($value));
            return;
        }
        $this->validateRange($shape, $value, "numeric value");
    }
    private function check_boolean(\WPCOM_VIP\Aws\Api\Shape $shape, $value)
    {
        if (!\is_bool($value)) {
            $this->addError('must be a boolean. Found ' . \WPCOM_VIP\Aws\describe_type($value));
        }
    }
    private function check_string(\WPCOM_VIP\Aws\Api\Shape $shape, $value)
    {
        if ($shape['jsonvalue']) {
            if (!self::canJsonEncode($value)) {
                $this->addError('must be a value encodable with \'json_encode\'.' . ' Found ' . \WPCOM_VIP\Aws\describe_type($value));
            }
            return;
        }
        if (!$this->checkCanString($value)) {
            $this->addError('must be a string or an object that implements ' . '__toString(). Found ' . \WPCOM_VIP\Aws\describe_type($value));
            return;
        }
    ...
    private function validateRange(\WPCOM_VIP\Aws\Api\Shape $shape, $length, $descriptor)
    {
    ...
    private function checkAssociativeArray($value)
    {
        ...
        if (!$isAssociative) {
            $this->addError('must be an associative array. Found ' . \WPCOM_VIP\Aws\describe_type($value));
            return \false;
        }
        return \true;
    }
