...
use WPCOM_VIP\Aws\Exception\UnresolvedApiException;
/**
 * API providers.
 *
...
class ApiProvider
{
    ...
    public static function resolve(callable $provider, $type, $service, $version)
    {
        ...
        throw new \WPCOM_VIP\Aws\Exception\UnresolvedApiException($msg);
    }
    /**
    ...
    {
        return new self(__DIR__ . '/../data', \WPCOM_VIP\Aws\manifest());
    }
    /**
    ...
    public function __invoke($type, $service, $version)
    {
        ...
        try {
            return \WPCOM_VIP\Aws\load_compiled_json($path);
