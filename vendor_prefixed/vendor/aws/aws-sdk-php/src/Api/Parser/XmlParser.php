...
class XmlParser
{
    public function parse(\WPCOM_VIP\Aws\Api\StructureShape $shape, \SimpleXMLElement $value)
    {
    ...
    private function parse_structure(\WPCOM_VIP\Aws\Api\StructureShape $shape, \SimpleXMLElement $value)
    {
        $target = [];
    ...
    private function memberKey(\WPCOM_VIP\Aws\Api\Shape $shape, $name)
    {
        if (null !== $shape['locationName']) {
            return $shape['locationName'];
        }
        if ($shape instanceof \WPCOM_VIP\Aws\Api\ListShape && $shape['flattened']) {
            return $shape->getMember()['locationName'] ?: $name;
        }
        return $name;
    }
    private function parse_list(\WPCOM_VIP\Aws\Api\ListShape $shape, \SimpleXMLElement $value)
    {
        $target = [];
    ...
    private function parse_map(\WPCOM_VIP\Aws\Api\MapShape $shape, \SimpleXMLElement $value)
    {
        $target = [];
        if (!$shape['flattened']) {
            $value = $value->entry;
    ...
    private function parse_blob(\WPCOM_VIP\Aws\Api\Shape $shape, $value)
    {
        return \base64_decode((string) $value);
    }
    private function parse_float(\WPCOM_VIP\Aws\Api\Shape $shape, $value)
    {
        return (float) (string) $value;
    }
    private function parse_integer(\WPCOM_VIP\Aws\Api\Shape $shape, $value)
    {
        return (int) (string) $value;
    }
    private function parse_boolean(\WPCOM_VIP\Aws\Api\Shape $shape, $value)
    {
        return $value == 'true';
    }
    private function parse_timestamp(\WPCOM_VIP\Aws\Api\Shape $shape, $value)
    {
        if (\is_string($value) || \is_int($value) || \is_object($value) && \method_exists($value, '__toString')) {
            return \WPCOM_VIP\Aws\Api\DateTimeResult::fromTimestamp((string) $value, !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : null);
        }
        throw new \WPCOM_VIP\Aws\Api\Parser\Exception\ParserException('Invalid timestamp value passed to XmlParser::parse_timestamp');
    }
    private function parse_xml_attribute(\WPCOM_VIP\Aws\Api\Shape $shape, \WPCOM_VIP\Aws\Api\Shape $memberShape, $value)
    {
