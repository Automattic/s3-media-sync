...
class QueryParamBuilder
{
    ...
    public function __invoke(\WPCOM_VIP\Aws\Api\StructureShape $shape, array $params)
    {
        if (!$this->methods) {
    ...
    protected function format(\WPCOM_VIP\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
    ...
    protected function format_structure(\WPCOM_VIP\Aws\Api\StructureShape $shape, array $value, $prefix, &$query)
    {
        if ($prefix) {
            $prefix .= '.';
        }
    ...
    protected function format_map(\WPCOM_VIP\Aws\Api\MapShape $shape, array $value, $prefix, array &$query)
    {
        $vals = $shape->getValue();
        $keys = $shape->getKey();
        if (!$this->isFlat($shape)) {
    ...
    protected function format_blob(\WPCOM_VIP\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
        $query[$prefix] = \base64_encode($value);
    }
    protected function format_timestamp(\WPCOM_VIP\Aws\Api\TimestampShape $shape, $value, $prefix, array &$query)
    {
        $timestampFormat = !empty($shape['timestampFormat']) ? $shape['timestampFormat'] : 'iso8601';
        $query[$prefix] = \WPCOM_VIP\Aws\Api\TimestampShape::format($value, $timestampFormat);
    }
    protected function format_boolean(\WPCOM_VIP\Aws\Api\Shape $shape, $value, $prefix, array &$query)
    {
