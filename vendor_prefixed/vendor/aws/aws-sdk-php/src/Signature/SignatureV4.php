...
use WPCOM_VIP\Aws\Exception\CouldNotCreateChecksumException;
 ...
 */
class SignatureV4 implements \WPCOM_VIP\Aws\Signature\SignatureInterface
{
    use SignatureTrait;
    const ISO8601_BASIC = 'WPCOM_VIP\\Ymd\\THis\\Z';
     ...
     */
    public static function convertPostToGet(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
        if ($request->getMethod() !== 'POST') {
            throw new \InvalidArgumentException('Expected a POST request but ' . 'received a ' . $request->getMethod() . ' request.');
        }
        $sr = $request->withMethod('GET')->withBody(\WPCOM_VIP\GuzzleHttp\Psr7\stream_for(''))->withoutHeader('Content-Type')->withoutHeader('Content-Length');
    ...
    protected function getPayload(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if (!$request->getBody()->isSeekable()) {
            throw new \WPCOM_VIP\Aws\Exception\CouldNotCreateChecksumException('sha256');
        }
        try {
            return \WPCOM_VIP\GuzzleHttp\Psr7\hash($request->getBody(), 'sha256');
        } catch (\Exception $e) {
            throw new \WPCOM_VIP\Aws\Exception\CouldNotCreateChecksumException('sha256', $e);
        }
    }
    protected function getPresignedPayload(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function createPresignedRequest(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, \WPCOM_VIP\Aws\Credentials\CredentialsInterface $credentials)
    {
    ...
    private function parseRequest(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $uri = $request->getUri();
        return ['method' => $request->getMethod(), 'path' => $uri->getPath(), 'query' => \WPCOM_VIP\GuzzleHttp\Psr7\parse_query($uri->getQuery()), 'uri' => $uri, 'headers' => $request->getHeaders(), 'body' => $request->getBody(), 'version' => $request->getProtocolVersion()];
    }
    private function buildRequest(array $req)
    {
        if ($req['query']) {
            $req['uri'] = $req['uri']->withQuery(\WPCOM_VIP\GuzzleHttp\Psr7\build_query($req['query']));
        }
        return new \WPCOM_VIP\GuzzleHttp\Psr7\Request($req['method'], $req['uri'], $req['headers'], $req['body'], $req['version']);
    }
}
