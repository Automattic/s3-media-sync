...
use WPCOM_VIP\Aws\Exception\UnresolvedSignatureException;
/**
 * Signature providers.
 *
...
class SignatureProvider
{
    ...
    public static function resolve(callable $provider, $version, $service, $region)
    {
        ...
        if ($result instanceof \WPCOM_VIP\Aws\Signature\SignatureInterface) {
            return $result;
        }
        throw new \WPCOM_VIP\Aws\Exception\UnresolvedSignatureException("Unable to resolve a signature for {$version}/{$service}/{$region}.\n" . "Valid signature versions include v4 and anonymous.");
    }
    /**
    ...
    public static function version()
    {
        return function ($version, $service, $region) {
            switch ($version) {
                case 's3v4':
                case 'v4':
                    return !empty(self::$s3v4SignedServices[$service]) ? new \WPCOM_VIP\Aws\Signature\S3SignatureV4($service, $region) : new \WPCOM_VIP\Aws\Signature\SignatureV4($service, $region);
                case 'v4-unsigned-body':
                    return new \WPCOM_VIP\Aws\Signature\SignatureV4($service, $region, ['unsigned-body' => 'true']);
                case 'anonymous':
                    return new \WPCOM_VIP\Aws\Signature\AnonymousSignature();
                default:
                    return null;
            }
        };
