...
class PartitionEndpointProvider
{
    ...
    public function __construct(array $partitions, $defaultPartition = 'aws', $options = [])
    {
        $this->partitions = \array_map(function (array $definition) {
            return new \WPCOM_VIP\Aws\Endpoint\Partition($definition);
    ...
    {
        $data = \WPCOM_VIP\Aws\load_compiled_json(__DIR__ . '/../data/endpoints.json');
        $prefixData = \WPCOM_VIP\Aws\load_compiled_json(__DIR__ . '/../data/endpoints_prefix_history.json');
    ...
    public static function mergePrefixData($data, $prefixData)
    {
        ...
        foreach ($data["partitions"] as $index => $partition) {
            foreach ($prefixGroups as $current => $old) {
                $serviceData = \WPCOM_VIP\JmesPath\Env::search("services.\"{$current}\"", $partition);
                if (!empty($serviceData)) {
                    foreach ($old as $prefix) {
                        if (empty(\WPCOM_VIP\JmesPath\Env::search("services.\"{$prefix}\"", $partition))) {
