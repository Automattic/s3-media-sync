 ...
 */
class GlacierClient extends \WPCOM_VIP\Aws\AwsClient
{
    public function __construct(array $args)
    {
        ...
        $stack->appendBuild(\WPCOM_VIP\Aws\Middleware::contentType(['UploadArchive', 'UploadPart']), 'glacier.content_type');
        $stack->appendInit(\WPCOM_VIP\Aws\Middleware::sourceFile($this->getApi(), 'body', 'sourceFile'), 'glacier.source_file');
    }
    /**
     * {@inheritdoc}
     *
        ...
        return function (callable $handler) {
            return function (\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                ...
                if (($name === 'UploadArchive' || $name === 'UploadMultipartPart') && (!$command['checksum'] || !$command['ContentSHA256'])) {
                    ...
                    if (!$command['checksum']) {
                        $body = new \WPCOM_VIP\Aws\HashingStream($body, new \WPCOM_VIP\Aws\Glacier\TreeHash(), function ($result) use(&$request) {
                    ...
                    if (!$command['ContentSHA256']) {
                        $body = new \WPCOM_VIP\Aws\HashingStream($body, new \WPCOM_VIP\Aws\PhpHash('sha256'), function ($result) use($command) {
