...
namespace WPCOM_VIP\Aws\Glacier;

use WPCOM_VIP\Aws\CommandInterface;
use WPCOM_VIP\Aws\HashingStream;
use WPCOM_VIP\Aws\Multipart\AbstractUploader;
use WPCOM_VIP\Aws\Multipart\UploadState;
use WPCOM_VIP\Aws\PhpHash;
 ...
 */
class MultipartUploader extends \WPCOM_VIP\Aws\Multipart\AbstractUploader
{
     ...
     * @return UploadState
     */
    public static function getStateFromService(\WPCOM_VIP\Aws\Glacier\GlacierClient $client, $vaultName, $uploadId, $accountId = '-')
    {
        $state = new \WPCOM_VIP\Aws\Multipart\UploadState(['accountId' => $accountId, 'vaultName' => $vaultName, 'uploadId' => $uploadId]);
     ...
     */
    public function __construct(\WPCOM_VIP\Aws\Glacier\GlacierClient $client, $source, array $config = [])
    {
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = \WPCOM_VIP\GuzzleHttp\Psr7\try_fopen($this->source->getMetadata('uri'), 'r');
            $body = $this->limitPartStream(\WPCOM_VIP\GuzzleHttp\Psr7\stream_for($body));
            ...
            $body = \WPCOM_VIP\GuzzleHttp\Psr7\stream_for();
            \WPCOM_VIP\GuzzleHttp\Psr7\copy_to_stream($source, $body);
        }
    ...
    {
        $treeHash = new \WPCOM_VIP\Aws\Glacier\TreeHash();
        $archiveSize = 0;
     ...
     * @return Stream
     */
    private function decorateWithHashes(\WPCOM_VIP\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        // Make sure that a tree hash is calculated.
        $stream = new \WPCOM_VIP\Aws\HashingStream($stream, new \WPCOM_VIP\Aws\Glacier\TreeHash(), function ($result) use(&$data) {
            $data['checksum'] = \bin2hex($result);
        });
        // Make sure that a linear SHA256 hash is calculated.
        $stream = new \WPCOM_VIP\Aws\HashingStream($stream, new \WPCOM_VIP\Aws\PhpHash('sha256'), function ($result) use(&$data) {
