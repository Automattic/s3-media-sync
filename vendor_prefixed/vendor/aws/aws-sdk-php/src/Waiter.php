 ...
 */
class Waiter implements \WPCOM_VIP\GuzzleHttp\Promise\PromisorInterface
{
     ...
     */
    public function __construct(\WPCOM_VIP\Aws\AwsClientInterface $client, $name, array $args = [], array $config = [])
    {
        $this->client = $client;
        $this->name = $name;
        $this->args = $args;
    ...
    public function promise()
    {
        return \WPCOM_VIP\GuzzleHttp\Promise\coroutine(function () {
            ...
            for ($state = 'retry', $attempt = 1; $state === 'retry'; $attempt++) {
                ...
                } catch (\WPCOM_VIP\Aws\Exception\AwsException $e) {
                    $result = $e;
                }
                ...
                if ($state === 'success') {
                    ...
                    (yield new \WPCOM_VIP\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException($msg)));
                } elseif ($state === 'retry' && $attempt >= $this->config['maxAttempts']) {
                    $state = 'failed';
                    (yield new \WPCOM_VIP\GuzzleHttp\Promise\RejectedPromise(new \RuntimeException("The {$this->name} waiter failed after attempt #{$attempt}.")));
                }
            }
        });
    }
    ...
    {
        return !$result instanceof \WPCOM_VIP\Aws\ResultInterface ? \false : $acceptor['expected'] == $result->search($acceptor['argument']);
    }
    /**
     * @param Result $result   Result or exception.
     * @param array  $acceptor Acceptor configuration being checked.
    ...
    {
        if (!$result instanceof \WPCOM_VIP\Aws\ResultInterface) {
            return \false;
        }
    ...
    private function matchesStatus($result, array $acceptor)
    {
        ...
        if ($result instanceof \WPCOM_VIP\Aws\Exception\AwsException && ($response = $result->getResponse())) {
