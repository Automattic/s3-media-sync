...
use WPCOM_VIP\Aws\CommandPool;
use WPCOM_VIP\Aws\Exception\AwsException;
use WPCOM_VIP\Aws\Exception\MultipartUploadException;
 ...
 * @internal
 */
abstract class AbstractUploadManager implements \WPCOM_VIP\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_CONCURRENCY = 5;
    /** @var array Default values for base multipart configuration */
    private static $defaultConfig = ['part_size' => null, 'state' => null, 'concurrency' => self::DEFAULT_CONCURRENCY, 'prepare_data_source' => null, 'before_initiate' => null, 'before_upload' => null, 'before_complete' => null, 'exception_class' => 'WPCOM_VIP\\Aws\\Exception\\MultipartUploadException'];
     ...
     * @param array  $config
     */
    public function __construct(\WPCOM_VIP\Aws\AwsClientInterface $client, array $config = [])
    {
        $this->client = $client;
    ...
    public function promise()
    {
        ...
        return $this->promise = \WPCOM_VIP\GuzzleHttp\Promise\coroutine(function () {
            // Initiate the upload.
            ...
            if (!$this->state->isInitiated()) {
                ...
                $this->state->setStatus(\WPCOM_VIP\Aws\Multipart\UploadState::INITIATED);
            }
            ...
            $commands = new \WPCOM_VIP\Aws\CommandPool($this->client, $this->getUploadCommands($resultHandler), ['concurrency' => $this->config['concurrency'], 'before' => $this->config['before_upload']]);
            ...
            $this->state->setStatus(\WPCOM_VIP\Aws\Multipart\UploadState::COMPLETED);
     ...
     */
    protected abstract function handleResult(\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Aws\ResultInterface $result);
    /**
    ...
    private function determineState()
    {
        ...
        if ($this->config['state'] instanceof \WPCOM_VIP\Aws\Multipart\UploadState) {
        ...
        $state = new \WPCOM_VIP\Aws\Multipart\UploadState($id);
    ...
    protected function getResultHandler(&$errors = [])
    {
        return function (callable $handler) use(&$errors) {
            return function (\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request = null) use($handler, &$errors) {
                return $handler($command, $request)->then(function (\WPCOM_VIP\Aws\ResultInterface $result) use($command) {
                    $this->handleResult($command, $result);
                    return $result;
                }, function (\WPCOM_VIP\Aws\Exception\AwsException $e) use(&$errors) {
                    $errors[$e->getCommand()[$this->info['part_num']]] = $e;
                    return new \WPCOM_VIP\Aws\Result();
                });
            };
        };
    }
