...
class LogRecordIterator implements \OuterIterator
{
     ...
     */
    public static function forTrail(\WPCOM_VIP\Aws\S3\S3Client $s3Client, \WPCOM_VIP\Aws\CloudTrail\CloudTrailClient $cloudTrailClient, array $options = [])
    {
        $logFileIterator = \WPCOM_VIP\Aws\CloudTrail\LogFileIterator::forTrail($s3Client, $cloudTrailClient, $options);
        return new self(new \WPCOM_VIP\Aws\CloudTrail\LogFileReader($s3Client), $logFileIterator);
    }
    /**
     * @param S3Client $s3Client
     ...
     * @return LogRecordIterator
     */
    public static function forBucket(\WPCOM_VIP\Aws\S3\S3Client $s3Client, $s3BucketName, array $options = [])
    {
        $logFileReader = new \WPCOM_VIP\Aws\CloudTrail\LogFileReader($s3Client);
        $iter = new \WPCOM_VIP\Aws\CloudTrail\LogFileIterator($s3Client, $s3BucketName, $options);
     ...
     * @return LogRecordIterator
     */
    public static function forFile(\WPCOM_VIP\Aws\S3\S3Client $s3Client, $s3BucketName, $s3ObjectKey)
    {
        $logFileReader = new \WPCOM_VIP\Aws\CloudTrail\LogFileReader($s3Client);
     ...
     */
    public function __construct(\WPCOM_VIP\Aws\CloudTrail\LogFileReader $logFileReader, \Iterator $logFileIterator)
    {
