 ...
 */
class CommandPool implements \WPCOM_VIP\GuzzleHttp\Promise\PromisorInterface
{
    /** @var EachPromise */
    private $each;
    /**
     ...
     */
    public function __construct(\WPCOM_VIP\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        ...
        $mapFn = function ($commands) use($client, $before, $config) {
            foreach ($commands as $key => $command) {
                if (!$command instanceof \WPCOM_VIP\Aws\CommandInterface) {
        ...
        $this->each = new \WPCOM_VIP\GuzzleHttp\Promise\EachPromise($mapFn($commands), $config);
    }
    /**
     ...
     */
    public static function batch(\WPCOM_VIP\Aws\AwsClientInterface $client, $commands, array $config = [])
    {
        $results = [];
