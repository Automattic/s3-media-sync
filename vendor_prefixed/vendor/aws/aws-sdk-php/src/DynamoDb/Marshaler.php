...
class Marshaler
{
    ...
    {
        return new \WPCOM_VIP\Aws\DynamoDb\BinaryValue($value);
    }
    /**
    ...
    {
        return new \WPCOM_VIP\Aws\DynamoDb\NumberValue($value);
    }
    /**
    ...
    {
        return new \WPCOM_VIP\Aws\DynamoDb\SetValue($values);
    }
    /**
    ...
    public function marshalValue($value)
    {
        ...
        if ($type === 'integer' || $type === 'double' || $value instanceof \WPCOM_VIP\Aws\DynamoDb\NumberValue) {
        ...
        // Handle set values.
        if ($value instanceof \WPCOM_VIP\Aws\DynamoDb\SetValue) {
            if (\count($value) === 0) {
        ...
        if (\is_resource($value) || $value instanceof \WPCOM_VIP\Psr\Http\Message\StreamInterface) {
            $value = $this->binary($value);
        }
        if ($value instanceof \WPCOM_VIP\Aws\DynamoDb\BinaryValue) {
    ...
    public function unmarshalValue(array $value, $mapAsObject = \false)
    {
        ...
        switch ($type) {
            ...
            case 'BS':
                ...
                return new \WPCOM_VIP\Aws\DynamoDb\SetValue($value);
        }
