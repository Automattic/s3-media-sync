...
use WPCOM_VIP\Aws\Api\Parser\Crc32ValidatingParser;
 ...
 */
class DynamoDbClient extends \WPCOM_VIP\Aws\AwsClient
{
    ...
    {
        $handler = \WPCOM_VIP\Aws\DynamoDb\SessionHandler::fromClient($this, $config);
        $handler->register();
        return $handler;
    }
    /** @internal */
    public static function _applyRetryConfig($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        if ($value) {
            $config = \WPCOM_VIP\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                $list->appendSign(\WPCOM_VIP\Aws\Middleware::retry(\WPCOM_VIP\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1, ['error_codes' => ['TransactionInProgressException']]), function ($retries) {
                    return $retries ? \WPCOM_VIP\Aws\RetryMiddleware::exponentialDelay($retries) / 2 : 0;
                }, isset($args['stats']['retries']) ? (bool) $args['stats']['retries'] : \false), 'retry');
            } else {
                $list->appendSign(\WPCOM_VIP\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries'], 'transient_error_codes' => ['TransactionInProgressException']]), 'retry');
            }
        }
    }
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        ...
        $args['parser'] = new \WPCOM_VIP\Aws\Api\Parser\Crc32ValidatingParser($args['parser']);
    }
}
