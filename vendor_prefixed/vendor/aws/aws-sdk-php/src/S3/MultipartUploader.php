...
use WPCOM_VIP\Aws\S3\Exception\S3MultipartUploadException;
/**
...
class MultipartUploader extends \WPCOM_VIP\Aws\Multipart\AbstractUploader
{
     ...
     */
    public function __construct(\WPCOM_VIP\Aws\S3\S3ClientInterface $client, $source, array $config = [])
    {
        parent::__construct($client, $source, \array_change_key_case($config) + ['bucket' => null, 'key' => null, 'exception_class' => \WPCOM_VIP\Aws\S3\Exception\S3MultipartUploadException::class]);
    }
    ...
    protected function createPart($seekable, $number)
    {
        ...
        if ($seekable) {
            ...
            $body = $this->limitPartStream(new \WPCOM_VIP\GuzzleHttp\Psr7\LazyOpenStream($this->source->getMetadata('uri'), 'r'));
        } else {
    ...
    protected function extractETag(\WPCOM_VIP\Aws\ResultInterface $result)
    {
        return $result['ETag'];
    }
    protected function getSourceMimeType()
    {
        if ($uri = $this->source->getMetadata('uri')) {
            return \WPCOM_VIP\GuzzleHttp\Psr7\mimetype_from_filename($uri) ?: 'application/octet-stream';
        }
    }
    ...
    private function decorateWithHashes(\WPCOM_VIP\Psr\Http\Message\StreamInterface $stream, array &$data)
    {
        ...
        $hash = new \WPCOM_VIP\Aws\PhpHash('sha256');
        return new \WPCOM_VIP\Aws\HashingStream($stream, $hash, function ($result) use(&$data) {
