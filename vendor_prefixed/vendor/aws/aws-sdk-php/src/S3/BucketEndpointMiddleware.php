...
class BucketEndpointMiddleware
{
    ...
    private function modifyRequest(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, \WPCOM_VIP\Aws\CommandInterface $command)
    {
        $uri = $request->getUri();
        $path = $uri->getPath();
        $bucket = $command['Bucket'];
        ...
        if ($command['Key']) {
            $path = \WPCOM_VIP\Aws\S3\S3Client::encodeKey(\rawurldecode($path));
        }
