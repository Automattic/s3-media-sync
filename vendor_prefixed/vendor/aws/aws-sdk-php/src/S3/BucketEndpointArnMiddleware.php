...
use WPCOM_VIP\Aws\Arn\ArnParser;
use WPCOM_VIP\Aws\Arn\Exception\InvalidArnException;
use WPCOM_VIP\Aws\Arn\S3\AccessPointArn;
...
class BucketEndpointArnMiddleware
{
     ...
     * @return callable
     */
    public static function wrap(\WPCOM_VIP\Aws\Api\Service $service, $region, array $config)
    {
        return function (callable $handler) use($service, $region, $config) {
            return new self($handler, $service, $region, $config);
        };
    }
    public function __construct(callable $nextHandler, \WPCOM_VIP\Aws\Api\Service $service, $region, array $config = [])
    {
        $this->partitionProvider = \WPCOM_VIP\Aws\Endpoint\PartitionEndpointProvider::defaultProvider();
        $this->region = $region;
        $this->service = $service;
        $this->config = $config;
        $this->nextHandler = $nextHandler;
    }
    public function __invoke(\WPCOM_VIP\Aws\CommandInterface $cmd, \WPCOM_VIP\Psr\Http\Message\RequestInterface $req)
    {
        ...
        if (!empty($op['input']['shape'])) {
            ...
            if (!empty($input = $service['shapes'][$op['input']['shape']])) {
                ...
                if (!empty($arnableKey) && \WPCOM_VIP\Aws\Arn\ArnParser::isArn($cmd[$arnableKey])) {
                    try {
                        // Throw for commands that do not support ARN inputs
                        if (\in_array($cmd->getName(), $this->nonArnableCommands)) {
                            throw new \WPCOM_VIP\Aws\S3\Exception\S3Exception('ARN values cannot be used in the bucket field for' . ' the ' . $cmd->getName() . ' operation.', $cmd);
                        }
                        $arn = \WPCOM_VIP\Aws\Arn\ArnParser::parse($cmd[$arnableKey]);
                    ...
                    } catch (\WPCOM_VIP\Aws\Arn\Exception\InvalidArnException $e) {
                        // Add context to ARN exception
                        throw new \WPCOM_VIP\Aws\S3\Exception\S3Exception('Bucket parameter parsed as ARN and failed with: ' . $e->getMessage(), $cmd, [], $e);
                    }
                }
            }
        }
    ...
    private function generateAccessPointHost(\WPCOM_VIP\Aws\Arn\S3\AccessPointArn $arn, \WPCOM_VIP\Psr\Http\Message\RequestInterface $req)
    {
    ...
    private function getPartitionSuffix(\WPCOM_VIP\Aws\Arn\ArnInterface $arn)
    {
    ...
    {
        $partition = \WPCOM_VIP\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartition($region, 's3');
    ...
    {
        if ($arn instanceof \WPCOM_VIP\Aws\Arn\S3\AccessPointArn) {
            // Accelerate is not supported with access points
            if (!empty($this->config['accelerate'])) {
                throw new \WPCOM_VIP\Aws\Exception\UnresolvedEndpointException('Accelerate is currently not supported with access points.' . ' Please disable accelerate or do not supply an access' . ' point ARN.');
            }
            // Path-style is not supported with access points
            if (!empty($this->config['path_style'])) {
                throw new \WPCOM_VIP\Aws\Exception\UnresolvedEndpointException('Path-style addressing is currently not supported with' . ' access points. Please disable path-style or do not' . ' supply an access point ARN.');
            }
            // Custom endpoint is not supported with access points
            if (!\is_null($this->config['endpoint'])) {
                throw new \WPCOM_VIP\Aws\Exception\UnresolvedEndpointException('A custom endpoint has been supplied along with an access' . ' point ARN, and these are not compatible with each other.' . ' Please only use one or the other.');
            }
            ...
            if ($arn->getPartition() !== $clientPart->getName()) {
                throw new \WPCOM_VIP\Aws\Exception\InvalidRegionException('The supplied ARN partition' . " does not match the client's partition.");
            }
            if ($clientPart->getName() !== $arnPart->getName()) {
                throw new \WPCOM_VIP\Aws\Exception\InvalidRegionException('The corresponding partition' . ' for the supplied ARN region does not match the' . " client's partition.");
            }
            ...
            if (!$this->isMatchingSigningRegion($arn->getRegion(), $this->region)) {
                if (empty($this->config['use_arn_region']) || !$this->config['use_arn_region']->isUseArnRegion()) {
                    throw new \WPCOM_VIP\Aws\Exception\InvalidRegionException('The region' . " specified in the ARN (" . $arn->getRegion() . ") does not match the client region (" . "{$this->region}).");
                }
            }
            return $arnPart;
        }
        throw new \WPCOM_VIP\Aws\Arn\Exception\InvalidArnException('Provided ARN was not' . ' a valid S3 access point ARN');
    }
}
