...
use WPCOM_VIP\Aws\S3\Exception\DeleteMultipleObjectsException;
 ...
 */
class BatchDelete implements \WPCOM_VIP\GuzzleHttp\Promise\PromisorInterface
{
    private $bucket;
     ...
     * @return BatchDelete
     */
    public static function fromListObjects(\WPCOM_VIP\Aws\AwsClientInterface $client, array $listObjectsParams, array $options = [])
    {
        $iter = $client->getPaginator('ListObjects', $listObjectsParams);
        $bucket = $listObjectsParams['Bucket'];
        $fn = function (\WPCOM_VIP\Aws\S3\BatchDelete $that) use($iter) {
            return $iter->each(function ($result) use($that) {
                ...
                return $promises ? \WPCOM_VIP\GuzzleHttp\Promise\all($promises) : null;
            });
        };
     ...
     * @return BatchDelete
     */
    public static function fromIterator(\WPCOM_VIP\Aws\AwsClientInterface $client, $bucket, \Iterator $iter, array $options = [])
    {
        $fn = function (\WPCOM_VIP\Aws\S3\BatchDelete $that) use($iter) {
            return \WPCOM_VIP\GuzzleHttp\Promise\coroutine(function () use($that, $iter) {
                foreach ($iter as $obj) {
     ...
     */
    private function __construct(\WPCOM_VIP\Aws\AwsClientInterface $client, $bucket, callable $promiseFn, array $options = [])
    {
        $this->client = $client;
        $this->bucket = $bucket;
    ...
    private function flushQueue()
    {
        ...
        return $this->client->executeAsync($command)->then(function ($result) {
            if (!empty($result['Errors'])) {
                throw new \WPCOM_VIP\Aws\S3\Exception\DeleteMultipleObjectsException($result['Deleted'] ?: [], $result['Errors']);
            }
            return $result;
        });
    }
    ...
    private function createPromise()
    {
        ...
        return $promise->then(function () use($cleanup) {
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for($this->flushQueue())->then($cleanup);
