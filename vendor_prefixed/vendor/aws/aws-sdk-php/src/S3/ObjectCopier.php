 ...
 * when appropriate.
 */
class ObjectCopier implements \WPCOM_VIP\GuzzleHttp\Promise\PromisorInterface
{
    const DEFAULT_MULTIPART_THRESHOLD = \WPCOM_VIP\Aws\S3\MultipartUploader::PART_MAX_SIZE;
    private $client;
    private $source;
    private $destination;
    private $acl;
     ...
     */
    public function __construct(\WPCOM_VIP\Aws\S3\S3ClientInterface $client, array $source, array $destination, $acl = 'private', array $options = [])
    {
    ...
    public function promise()
    {
        return \WPCOM_VIP\GuzzleHttp\Promise\coroutine(function () {
            ...
            if ($objectStats['ContentLength'] > $this->options['mup_threshold']) {
                $mup = new \WPCOM_VIP\Aws\S3\MultipartCopy($this->client, $this->getSourcePath(), ['source_metadata' => $objectStats, 'acl' => $this->acl] + $this->destination + $this->options);
                (yield $mup->promise());
            } else {
    ...
    {
        if (\WPCOM_VIP\Aws\Arn\ArnParser::isArn($this->source['Bucket'])) {
            try {
                new \WPCOM_VIP\Aws\Arn\S3\AccessPointArn($this->source['Bucket']);
            } catch (\Exception $e) {
