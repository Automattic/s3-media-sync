...
use WPCOM_VIP\Aws\S3\MultipartUploader;
 ...
 * @deprecated
 */
class S3EncryptionMultipartUploader extends \WPCOM_VIP\Aws\S3\MultipartUploader
{
    ...
    {
        return \in_array($cipherName, \WPCOM_VIP\Aws\Crypto\AbstractCryptoClient::$supportedCiphers);
    }
    private $provider;
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = \WPCOM_VIP\GuzzleHttp\Promise\promise_for($this->encrypt($this->source, $this->config['@cipheroptions'] ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
