 ...
 */
class S3EncryptionMultipartUploaderV2 extends \WPCOM_VIP\Aws\S3\MultipartUploader
{
    ...
    {
        return \in_array($cipherName, \WPCOM_VIP\Aws\Crypto\AbstractCryptoClientV2::$supportedCiphers);
    }
    private $provider;
    ...
    private function getEncryptingDataPreparer()
    {
        return function () {
            ...
            list($this->source, $params) = \WPCOM_VIP\GuzzleHttp\Promise\promise_for($this->encrypt($this->source, $this->config ?: [], $this->provider, $envelope))->then(function ($bodyStream) use($envelope) {
