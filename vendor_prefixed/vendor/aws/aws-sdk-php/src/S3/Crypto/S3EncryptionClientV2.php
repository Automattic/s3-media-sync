 ...
 * </code>
 */
class S3EncryptionClientV2 extends \WPCOM_VIP\Aws\Crypto\AbstractCryptoClientV2
{
     ...
     */
    public function __construct(\WPCOM_VIP\Aws\S3\S3Client $client, $instructionFileSuffix = null)
    {
    ...
    public function putObjectAsync(array $args)
    {
        ...
        $envelope = new \WPCOM_VIP\Aws\Crypto\MetadataEnvelope();
        return \WPCOM_VIP\GuzzleHttp\Promise\promise_for($this->encrypt(\WPCOM_VIP\GuzzleHttp\Psr7\stream_for($args['Body']), $args, $provider, $envelope))->then(function ($encryptedBodyStream) use($args) {
            $hash = new \WPCOM_VIP\Aws\PhpHash('sha256');
            $hashingEncryptedBodyStream = new \WPCOM_VIP\Aws\HashingStream($encryptedBodyStream, $hash, self::getContentShaDecorator($args));
    ...
    public function getObjectAsync(array $args)
    {
        ...
        if (!isset($args['@SecurityProfile']) || !\in_array($args['@SecurityProfile'], self::$supportedSecurityProfiles)) {
            throw new \WPCOM_VIP\Aws\Exception\CryptoException("@SecurityProfile is required and must be" . " set to 'V2' or 'V2_AND_LEGACY'");
        }
