...
class S3UriParser
{
    ...
    public function parse($uri)
    {
        ...
        if (!empty($components)) {
            if (\WPCOM_VIP\Aws\Arn\ArnParser::isArn($components['host'])) {
                $arn = new \WPCOM_VIP\Aws\Arn\S3\AccessPointArn($components['host']);
                return ['bucket' => $components['host'], 'key' => $components['path'], 'path_style' => \false, 'region' => $arn->getRegion()];
            }
        }
        $url = \WPCOM_VIP\GuzzleHttp\Psr7\uri_for($uri);
    ...
    private function parseStreamWrapper(\WPCOM_VIP\Psr\Http\Message\UriInterface $url)
    {
    ...
    private function parseCustomEndpoint(\WPCOM_VIP\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
    ...
    private function parsePathStyle(\WPCOM_VIP\Psr\Http\Message\UriInterface $url)
    {
        $result = self::$defaultResult;
        if ($url->getPath() != '/') {
            $path = \ltrim($url->getPath(), '/');
    ...
    private function parseVirtualHosted(\WPCOM_VIP\Psr\Http\Message\UriInterface $url, array $matches)
    {
        $result = self::$defaultResult;
        $result['path_style'] = \false;
