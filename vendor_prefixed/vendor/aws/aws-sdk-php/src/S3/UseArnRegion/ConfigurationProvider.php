...
class ConfigurationProvider extends \WPCOM_VIP\Aws\AbstractConfigurationProvider implements \WPCOM_VIP\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPCOM_VIP\Aws\S3\UseArnRegion\ConfigurationInterface::class;
    protected static $exceptionClass = \WPCOM_VIP\Aws\S3\UseArnRegion\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (isset($config['use_arn_region']) && $config['use_arn_region'] instanceof \WPCOM_VIP\Aws\CacheInterface) {
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($useArnRegion)) {
                return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\S3\UseArnRegion\Configuration($useArnRegion));
            }
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \WPCOM_VIP\Aws\parse_ini_file($filename, \true, \INI_SCANNER_NORMAL);
            if ($data === \false) {
            ...
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\S3\UseArnRegion\Configuration($data[$profile][self::INI_USE_ARN_REGION]));
        };
    }
    /**
    ...
    {
        return function () {
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\S3\UseArnRegion\Configuration(self::DEFAULT_USE_ARN_REGION));
        };
    }
}
