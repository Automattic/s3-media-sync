...
use WPCOM_VIP\Aws\MultiRegionClient as BaseClient;
 ...
 */
class S3MultiRegionClient extends \WPCOM_VIP\Aws\MultiRegionClient implements \WPCOM_VIP\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    /** @var CacheInterface */
    private $cache;
    public static function getArguments()
    {
        ...
        unset($args['region']);
        return $args + ['bucket_region_cache' => ['type' => 'config', 'valid' => [\WPCOM_VIP\Aws\CacheInterface::class], 'doc' => 'Cache of regions in which given buckets are located.', 'default' => function () {
            return new \WPCOM_VIP\Aws\LruArrayCache();
        }], 'region' => $regionDef];
    }
    ...
    private function determineRegionMiddleware()
    {
        return function (callable $handler) {
            return function (\WPCOM_VIP\Aws\CommandInterface $command) use($handler) {
                $cacheKey = $this->getCacheKey($command['Bucket']);
                if (empty($command['@region']) && ($region = $this->cache->get($cacheKey))) {
                    $command['@region'] = $region;
                }
                return \WPCOM_VIP\GuzzleHttp\Promise\coroutine(function () use($handler, $command, $cacheKey) {
                    try {
                        (yield $handler($command));
                    } catch (\WPCOM_VIP\Aws\S3\Exception\PermanentRedirectException $e) {
    ...
    public function determineBucketRegionAsync($bucketName)
    {
        ...
        if ($cached = $this->cache->get($cacheKey)) {
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for($cached);
        }
