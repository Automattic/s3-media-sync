...
use WPCOM_VIP\Aws\Signature\SignatureTrait;
use WPCOM_VIP\Aws\Signature\SignatureV4 as SignatureV4;
use WPCOM_VIP\Aws\Api\TimestampShape as TimestampShape;
/**
...
class PostObjectV4
{
     ...
     */
    public function __construct(\WPCOM_VIP\Aws\S3\S3ClientInterface $client, $bucket, array $formInputs, array $options = [], $expiration = '+1 hours')
    {
        $this->client = $client;
        $this->bucket = $bucket;
        ...
        // setup basic policy
        $policy = ['expiration' => \WPCOM_VIP\Aws\Api\TimestampShape::format($expiration, 'iso8601'), 'conditions' => $options];
    ...
    {
        $uri = new \WPCOM_VIP\GuzzleHttp\Psr7\Uri($this->client->getEndpoint());
    ...
    protected function getPolicyAndSignature(\WPCOM_VIP\Aws\Credentials\CredentialsInterface $credentials, array $policy)
    {
        $ldt = \gmdate(\WPCOM_VIP\Aws\Signature\SignatureV4::ISO8601_BASIC);
        $sdt = \substr($ldt, 0, 8);
