...
use WPCOM_VIP\Aws\Multipart\AbstractUploadManager;
use WPCOM_VIP\Aws\ResultInterface;
use WPCOM_VIP\GuzzleHttp\Psr7;
class MultipartCopy extends \WPCOM_VIP\Aws\Multipart\AbstractUploadManager
{
    ...
    {
        if (\WPCOM_VIP\Aws\Arn\ArnParser::isArn($source)) {
            $this->source = '';
        } else {
            $this->source = "/";
        }
    ...
    {
        if ($this->config['source_metadata'] instanceof \WPCOM_VIP\Aws\ResultInterface) {
        ...
        if (\strpos($key, '?')) {
            ...
            $headParams['Key'] = $key;
            $query = \WPCOM_VIP\GuzzleHttp\Psr7\parse_query($query, \false);
