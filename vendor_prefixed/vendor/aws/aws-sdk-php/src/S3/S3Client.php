...
use WPCOM_VIP\Aws\Api\DocModel;
use WPCOM_VIP\Aws\Api\Service;
use WPCOM_VIP\Aws\AwsClient;
use WPCOM_VIP\Aws\CacheInterface;
use WPCOM_VIP\Aws\ClientResolver;
use WPCOM_VIP\Aws\Command;
use WPCOM_VIP\Aws\Exception\AwsException;
use WPCOM_VIP\Aws\HandlerList;
use WPCOM_VIP\Aws\Middleware;
use WPCOM_VIP\Aws\Retry\QuotaManager;
use WPCOM_VIP\Aws\RetryMiddleware;
use WPCOM_VIP\Aws\ResultInterface;
use WPCOM_VIP\Aws\CommandInterface;
use WPCOM_VIP\Aws\RetryMiddlewareV2;
use WPCOM_VIP\Aws\S3\UseArnRegion\Configuration;
use WPCOM_VIP\Aws\S3\UseArnRegion\ConfigurationInterface;
use WPCOM_VIP\Aws\S3\UseArnRegion\ConfigurationProvider as UseArnRegionConfigurationProvider;
use WPCOM_VIP\Aws\S3\RegionalEndpoint\ConfigurationProvider;
use WPCOM_VIP\GuzzleHttp\Exception\RequestException;
use WPCOM_VIP\GuzzleHttp\Promise\Promise;
 ...
 */
class S3Client extends \WPCOM_VIP\Aws\AwsClient implements \WPCOM_VIP\Aws\S3\S3ClientInterface
{
    use S3ClientTrait;
    ...
    {
        if (!isset($args['s3_us_east_1_regional_endpoint']) || $args['s3_us_east_1_regional_endpoint'] instanceof \WPCOM_VIP\Aws\CacheInterface) {
            $args['s3_us_east_1_regional_endpoint'] = \WPCOM_VIP\Aws\S3\RegionalEndpoint\ConfigurationProvider::defaultProvider($args);
        }
        parent::__construct($args);
        $stack = $this->getHandlerList();
        $stack->appendInit(\WPCOM_VIP\Aws\S3\SSECMiddleware::wrap($this->getEndpoint()->getScheme()), 's3.ssec');
        $stack->appendBuild(\WPCOM_VIP\Aws\S3\ApplyChecksumMiddleware::wrap($this->getApi()), 's3.checksum');
        $stack->appendBuild(\WPCOM_VIP\Aws\Middleware::contentType(['PutObject', 'UploadPart']), 's3.content_type');
        // Use the bucket style middleware when using a "bucket_endpoint" (for cnames)
        if ($this->getConfig('bucket_endpoint')) {
            $stack->appendBuild(\WPCOM_VIP\Aws\S3\BucketEndpointMiddleware::wrap(), 's3.bucket_endpoint');
        } else {
            $stack->appendBuild(\WPCOM_VIP\Aws\S3\S3EndpointMiddleware::wrap($this->getRegion(), $this->getConfig('endpoint_provider'), ['dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint')]), 's3.endpoint_middleware');
        }
        $stack->appendBuild(\WPCOM_VIP\Aws\S3\BucketEndpointArnMiddleware::wrap($this->getApi(), $this->getRegion(), ['use_arn_region' => $this->getConfig('use_arn_region'), 'dual_stack' => $this->getConfig('use_dual_stack_endpoint'), 'accelerate' => $this->getConfig('use_accelerate_endpoint'), 'path_style' => $this->getConfig('use_path_style_endpoint'), 'endpoint' => isset($args['endpoint']) ? $args['endpoint'] : null]), 's3.bucket_endpoint_arn');
        $stack->appendSign(\WPCOM_VIP\Aws\S3\PutObjectUrlMiddleware::wrap(), 's3.put_object_url');
        $stack->appendSign(\WPCOM_VIP\Aws\S3\PermanentRedirectMiddleware::wrap(), 's3.permanent_redirect');
        $stack->appendInit(\WPCOM_VIP\Aws\Middleware::sourceFile($this->getApi()), 's3.source_file');
    ...
    public static function _apply_use_arn_region($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        if ($value instanceof \WPCOM_VIP\Aws\CacheInterface) {
            $value = \WPCOM_VIP\Aws\S3\UseArnRegion\ConfigurationProvider::defaultProvider($args);
        }
        if (\is_callable($value)) {
            $value = $value();
        }
        if ($value instanceof \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface) {
            $value = $value->wait();
        }
        if ($value instanceof \WPCOM_VIP\Aws\S3\UseArnRegion\ConfigurationInterface) {
            $args['use_arn_region'] = $value;
        } else {
            // The Configuration class itself will validate other inputs
            $args['use_arn_region'] = new \WPCOM_VIP\Aws\S3\UseArnRegion\Configuration($value);
        }
    }
    public function createPresignedRequest(\WPCOM_VIP\Aws\CommandInterface $command, $expires, array $options = [])
    {
        ...
        return $signer->presign(\WPCOM_VIP\Aws\serialize($command), $this->getCredentials()->wait(), $expires, $options);
    }
    /**
    ...
    public function getObjectUrl($bucket, $key)
    {
        ...
        return (string) \WPCOM_VIP\Aws\serialize($command)->getUri();
    }
    /**
    ...
    private function getLocationConstraintMiddleware()
    {
        ...
        return static function (callable $handler) use($region) {
            return function (\WPCOM_VIP\Aws\Command $command, $request = null) use($handler, $region) {
    ...
    private function getSaveAsParameter()
    {
        return static function (callable $handler) {
            return function (\WPCOM_VIP\Aws\Command $command, $request = null) use($handler) {
    ...
    {
        return static function (callable $handler) {
            return function (\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request = null) use($handler) {
    ...
    private function getEncodingTypeMiddleware()
    {
        return static function (callable $handler) {
            return function (\WPCOM_VIP\Aws\Command $command, $request = null) use($handler) {
                ...
                return $handler($command, $request)->then(function (\WPCOM_VIP\Aws\ResultInterface $result) use($autoSet) {
    ...
    /** @internal */
    public static function _applyRetryConfig($value, $args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        if ($value) {
            $config = \WPCOM_VIP\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                $maxRetries = $config->getMaxAttempts() - 1;
                $decider = \WPCOM_VIP\Aws\RetryMiddleware::createDefaultDecider($maxRetries);
                $decider = function ($retries, $command, $request, $result, $error) use($decider, $maxRetries) {
                    ...
                    if ($error instanceof \WPCOM_VIP\Aws\Exception\AwsException && $retries < $maxRetries) {
                        ...
                        if ($error->getPrevious() instanceof \WPCOM_VIP\GuzzleHttp\Exception\RequestException) {
                ...
                $delay = [\WPCOM_VIP\Aws\RetryMiddleware::class, 'exponentialDelay'];
                $list->appendSign(\WPCOM_VIP\Aws\Middleware::retry($decider, $delay), 'retry');
            } else {
                $defaultDecider = \WPCOM_VIP\Aws\RetryMiddlewareV2::createDefaultDecider(new \WPCOM_VIP\Aws\Retry\QuotaManager(), $config->getMaxAttempts());
                $list->appendSign(\WPCOM_VIP\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries'], 'decider' => function ($attempts, \WPCOM_VIP\Aws\CommandInterface $cmd, $result) use($defaultDecider, $config) {
                    $isRetryable = $defaultDecider($attempts, $cmd, $result);
                    if (!$isRetryable && $result instanceof \WPCOM_VIP\Aws\Exception\AwsException && $attempts < $config->getMaxAttempts()) {
                        if (!empty($result->getResponse()) && \strpos($result->getResponse()->getBody(), 'Your socket connection to the server') !== \false) {
                            $isRetryable = \false;
                        }
                        if ($result->getPrevious() instanceof \WPCOM_VIP\GuzzleHttp\Exception\RequestException && $cmd->getName() !== 'CompleteMultipartUpload') {
                            $isRetryable = \true;
                        }
                    }
                    return $isRetryable;
    ...
    /** @internal */
    public static function _applyApiProvider($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        \WPCOM_VIP\Aws\ClientResolver::_apply_api_provider($value, $args);
        $args['parser'] = new \WPCOM_VIP\Aws\S3\GetBucketLocationParser(new \WPCOM_VIP\Aws\S3\AmbiguousSuccessParser(new \WPCOM_VIP\Aws\S3\RetryableMalformedResponseParser($args['parser'], $args['exception_class']), $args['error_parser'], $args['exception_class']));
    }
    /**
     * @internal
     * @codeCoverageIgnore
    ...
    public static function applyDocFilters(array $api, array $docs)
    {
        ...
        return [new \WPCOM_VIP\Aws\Api\Service($api, \WPCOM_VIP\Aws\Api\ApiProvider::defaultProvider()), new \WPCOM_VIP\Aws\Api\DocModel($docs)];
    }
    /**
     * @internal
     * @codeCoverageIgnore
