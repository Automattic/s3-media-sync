...
class S3EndpointMiddleware
{
    ...
    public function __construct(callable $nextHandler, $region, array $options, $endpointProvider = null)
    {
        ...
        $this->endpointProvider = \is_null($endpointProvider) ? \WPCOM_VIP\Aws\Endpoint\PartitionEndpointProvider::defaultProvider() : $endpointProvider;
    ...
    private static function isRequestHostStyleCompatible(\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
        return \WPCOM_VIP\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']) && ($request->getUri()->getScheme() === 'http' || \strpos($command['Bucket'], '.') === \false) && \filter_var($request->getUri()->getHost(), \FILTER_VALIDATE_IP) === \false;
    }
    private function endpointPatternDecider(\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function canAccelerate(\WPCOM_VIP\Aws\CommandInterface $command)
    {
        return empty(self::$exclusions[$command->getName()]) && \WPCOM_VIP\Aws\S3\S3Client::isBucketDnsCompatible($command['Bucket']);
    }
    private function getBucketStyleHost(\WPCOM_VIP\Aws\CommandInterface $command, $host)
    {
    ...
    private function applyHostStyleEndpoint(\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
        $uri = $request->getUri();
    ...
    private function applyDualStackEndpoint(\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
    ...
    private function applyAccelerateEndpoint(\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request, $pattern)
    {
    ...
    private function getAccelerateHost(\WPCOM_VIP\Aws\CommandInterface $command, $pattern)
    {
        $dnsSuffix = $this->endpointProvider->getPartition($this->region, 's3')->getDnsSuffix();
        return "{$command['Bucket']}.{$pattern}.{$dnsSuffix}";
    }
    private function getBucketlessPath($path, \WPCOM_VIP\Aws\CommandInterface $command)
    {
