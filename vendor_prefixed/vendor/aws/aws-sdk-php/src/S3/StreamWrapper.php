...
class StreamWrapper
{
     ...
     */
    public static function register(\WPCOM_VIP\Aws\S3\S3ClientInterface $client, $protocol = 's3', \WPCOM_VIP\Aws\CacheInterface $cache = null)
    {
        ...
        if ($cache) {
            ...
            $default[$protocol]['cache'] = new \WPCOM_VIP\Aws\LruArrayCache();
        }
    ...
    public function stream_flush()
    {
        ...
        if (!isset($params['ContentType']) && ($type = \WPCOM_VIP\GuzzleHttp\Psr7\mimetype_from_filename($params['Key']))) {
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $this->objectIterator = \WPCOM_VIP\Aws\flatmap($this->getClient()->getPaginator('ListObjects', $op), function (\WPCOM_VIP\Aws\Result $result) use($filterFn) {
    ...
    private function openReadStream()
    {
        ...
        if ($this->getOption('seekable') && !$this->body->isSeekable()) {
            $this->body = new \WPCOM_VIP\GuzzleHttp\Psr7\CachingStream($this->body);
        }
        return \true;
    }
    private function openWriteStream()
    {
        $this->body = new \WPCOM_VIP\GuzzleHttp\Psr7\Stream(\fopen('php://temp', 'r+'));
        return \true;
    }
    ...
    {
        if (!$this->cache) {
            $this->cache = $this->getOption('cache') ?: new \WPCOM_VIP\Aws\LruArrayCache();
        }
        return $this->cache;
    }
    /**
