...
class MockHandler implements \Countable
{
    ...
    public function append()
    {
        foreach (\func_get_args() as $value) {
            if ($value instanceof \WPCOM_VIP\Aws\ResultInterface || $value instanceof \WPCOM_VIP\Aws\Exception\AwsException || \is_callable($value)) {
                $this->queue[] = $value;
            } else {
    ...
    public function __invoke(\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
        ...
        if ($result instanceof \Exception) {
            $result = new \WPCOM_VIP\GuzzleHttp\Promise\RejectedPromise($result);
        } else {
            ...
            $result = \WPCOM_VIP\GuzzleHttp\Promise\promise_for($result);
        }
