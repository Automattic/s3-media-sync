...
use WPCOM_VIP\GuzzleHttp\Event\EndEvent;
use WPCOM_VIP\GuzzleHttp\Exception\ConnectException;
use WPCOM_VIP\GuzzleHttp\Exception\RequestException;
use WPCOM_VIP\GuzzleHttp\Message\ResponseInterface as GuzzleResponse;
use WPCOM_VIP\GuzzleHttp\Promise;
use WPCOM_VIP\GuzzleHttp\Psr7\Response as Psr7Response;
use WPCOM_VIP\GuzzleHttp\Stream\Stream;
...
class GuzzleHandler
{
    ...
    public function __invoke(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options = [])
    {
        ...
        $promise = new \WPCOM_VIP\GuzzleHttp\Promise\Promise(function () use($guzzlePromise) {
            try {
                $guzzlePromise->wait();
            } catch (\Exception $e) {
        ...
        return $promise->then(function (\WPCOM_VIP\GuzzleHttp\Message\ResponseInterface $response) {
            ...
            if ($exception instanceof \WPCOM_VIP\GuzzleHttp\Exception\RequestException) {
                if (isset($options['sink'])) {
                    if (!$options['sink'] instanceof \WPCOM_VIP\Psr\Http\Message\StreamInterface) {
                        $exception->getResponse()->setBody(\WPCOM_VIP\GuzzleHttp\Stream\Stream::factory(\file_get_contents($options['sink'])));
                    }
                }
            }
            // Reject with information about the error.
            return new \WPCOM_VIP\GuzzleHttp\Promise\RejectedPromise($this->prepareErrorData($exception));
        });
    }
    private function createGuzzleRequest(\WPCOM_VIP\Psr\Http\Message\RequestInterface $psrRequest, array $options)
    {
        $ringConfig = [];
        ...
        // Prepare sink option.
        if (isset($options['sink'])) {
            $ringConfig['save_to'] = $options['sink'] instanceof \WPCOM_VIP\Psr\Http\Message\StreamInterface ? new \WPCOM_VIP\Aws\Handler\GuzzleV5\GuzzleStream($options['sink']) : $options['sink'];
            unset($options['sink']);
        }
        ...
        if (\is_callable($statsCallback)) {
            $request->getEmitter()->on('end', function (\WPCOM_VIP\GuzzleHttp\Event\EndEvent $event) use($statsCallback) {
        ...
        if ($body->getSize() === 0) {
            ...
            $request->setBody(new \WPCOM_VIP\Aws\Handler\GuzzleV5\GuzzleStream($body));
        }
        $request->setHeaders($psrRequest->getHeaders());
        $request->setHeader('User-Agent', $request->getHeader('User-Agent') . ' ' . \WPCOM_VIP\GuzzleHttp\Client::getDefaultUserAgent());
    ...
    private function createPsr7Response(\WPCOM_VIP\GuzzleHttp\Message\ResponseInterface $response)
    {
        if ($body = $response->getBody()) {
            $body = new \WPCOM_VIP\Aws\Handler\GuzzleV5\PsrStream($body);
        }
        return new \WPCOM_VIP\GuzzleHttp\Psr7\Response($response->getStatusCode(), $response->getHeaders(), $body, $response->getReasonPhrase());
    }
    private function prepareErrorData(\Exception $e)
    {
        $error = ['exception' => $e, 'connection_error' => \false, 'response' => null];
        if ($e instanceof \WPCOM_VIP\GuzzleHttp\Exception\ConnectException) {
