 ...
 * @internal
 */
class ApiCallMonitoringMiddleware extends \WPCOM_VIP\Aws\ClientSideMonitoring\AbstractMonitoringMiddleware
{
    /**
    ...
    private static function getResultAttemptCount(\WPCOM_VIP\Aws\ResultInterface $result)
    {
    ...
    {
        $attemptCount = 0;
        if ($e instanceof \WPCOM_VIP\Aws\MonitoringEventsInterface) {
    ...
    {
        $data = [];
        if ($klass instanceof \WPCOM_VIP\Aws\MonitoringEventsInterface) {
    ...
    {
        if ($klass instanceof \WPCOM_VIP\Aws\Exception\AwsException && $klass->isMaxRetriesExceeded()) {
            return 1;
        }
        return 0;
    }
