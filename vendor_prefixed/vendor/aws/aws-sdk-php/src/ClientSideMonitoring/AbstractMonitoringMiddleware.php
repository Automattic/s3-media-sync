 ...
 * @internal
 */
abstract class AbstractMonitoringMiddleware implements \WPCOM_VIP\Aws\ClientSideMonitoring\MonitoringMiddlewareInterface
{
    ...
    protected $region;
    protected $service;
    protected static function getAwsExceptionHeader(\WPCOM_VIP\Aws\Exception\AwsException $e, $headerName)
    {
    ...
    protected static function getResultHeader(\WPCOM_VIP\Aws\ResultInterface $result, $headerName)
    {
    ...
    public function __invoke(\WPCOM_VIP\Aws\CommandInterface $cmd, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
        ...
        $g = function ($value) use($eventData, $enabled) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                return \WPCOM_VIP\GuzzleHttp\Promise\rejection_for($value);
            }
            return $value;
        };
        return \WPCOM_VIP\GuzzleHttp\Promise\promise_for($handler($cmd, $request))->then($g, $g);
    }
    private function getClientId()
    {
        return $this->unwrappedOptions()->getClientId();
    }
    private function getNewEvent(\WPCOM_VIP\Aws\CommandInterface $cmd, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
        $event = ['Api' => $cmd->getName(), 'ClientId' => $this->getClientId(), 'Region' => $this->getRegion(), 'Service' => $this->getService(), 'Timestamp' => (int) \floor(\microtime(\true) * 1000), 'UserAgent' => \substr($request->getHeaderLine('User-Agent') . ' ' . \WPCOM_VIP\Aws\default_user_agent(), 0, 256), 'Version' => 1];
        return $event;
    }
    ...
    {
        if (!$this->options instanceof \WPCOM_VIP\Aws\ClientSideMonitoring\ConfigurationInterface) {
            try {
                $this->options = \WPCOM_VIP\Aws\ClientSideMonitoring\ConfigurationProvider::unwrap($this->options);
            } catch (\Exception $e) {
                // Errors unwrapping CSM config defaults to disabling it
                $this->options = new \WPCOM_VIP\Aws\ClientSideMonitoring\Configuration(\false, \WPCOM_VIP\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \WPCOM_VIP\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT);
            }
        }
        return $this->options;
    }
