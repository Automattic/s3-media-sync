...
use WPCOM_VIP\Aws\AbstractConfigurationProvider;
use WPCOM_VIP\Aws\CacheInterface;
use WPCOM_VIP\Aws\ClientSideMonitoring\Exception\ConfigurationException;
use WPCOM_VIP\Aws\ConfigurationProviderInterface;
 ...
 * </code>
 */
class ConfigurationProvider extends \WPCOM_VIP\Aws\AbstractConfigurationProvider implements \WPCOM_VIP\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPCOM_VIP\Aws\ClientSideMonitoring\ConfigurationInterface::class;
    protected static $exceptionClass = \WPCOM_VIP\Aws\ClientSideMonitoring\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (isset($config['csm']) && $config['csm'] instanceof \WPCOM_VIP\Aws\CacheInterface) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($enabled !== \false) {
                return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\ClientSideMonitoring\Configuration($enabled, \getenv(self::ENV_HOST) ?: self::DEFAULT_HOST, \getenv(self::ENV_PORT) ?: self::DEFAULT_PORT, \getenv(self::ENV_CLIENT_ID) ?: self::DEFAULT_CLIENT_ID));
            }
    ...
    {
        return function () {
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\ClientSideMonitoring\Configuration(self::DEFAULT_ENABLED, self::DEFAULT_HOST, self::DEFAULT_PORT, self::DEFAULT_CLIENT_ID));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \WPCOM_VIP\Aws\parse_ini_file($filename, \true);
            if ($data === \false) {
            ...
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\ClientSideMonitoring\Configuration($data[$profile]['csm_enabled'], $data[$profile]['csm_host'], $data[$profile]['csm_port'], $data[$profile]['csm_client_id']));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface) {
            $config = $config->wait();
        }
        if ($config instanceof \WPCOM_VIP\Aws\ClientSideMonitoring\ConfigurationInterface) {
            return $config;
            ...
            return new \WPCOM_VIP\Aws\ClientSideMonitoring\Configuration($config['enabled'], $host, $port, $client_id);
        }
