 ...
 */
class StsClient extends \WPCOM_VIP\Aws\AwsClient
{
    /**
     * {@inheritdoc}
     *
    ...
    {
        if (!isset($args['sts_regional_endpoints']) || $args['sts_regional_endpoints'] instanceof \WPCOM_VIP\Aws\CacheInterface) {
            $args['sts_regional_endpoints'] = \WPCOM_VIP\Aws\Sts\RegionalEndpoints\ConfigurationProvider::defaultProvider($args);
        }
     ...
     */
    public function createCredentials(\WPCOM_VIP\Aws\Result $result)
    {
        if (!$result->hasKey('Credentials')) {
            throw new \InvalidArgumentException('Result contains no credentials');
        }
        $c = $result['Credentials'];
        return new \WPCOM_VIP\Aws\Credentials\Credentials($c['AccessKeyId'], $c['SecretAccessKey'], isset($c['SessionToken']) ? $c['SessionToken'] : null, isset($c['Expiration']) && $c['Expiration'] instanceof \DateTimeInterface ? (int) $c['Expiration']->format('U') : null);
    }
}
