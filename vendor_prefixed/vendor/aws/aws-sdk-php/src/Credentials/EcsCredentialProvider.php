...
class EcsCredentialProvider
{
    ...
    {
        $client = $this->client;
        $request = new \WPCOM_VIP\GuzzleHttp\Psr7\Request('GET', self::getEcsUri());
        return $client($request, ['timeout' => $this->timeout, 'proxy' => ''])->then(function (\WPCOM_VIP\Psr\Http\Message\ResponseInterface $response) {
            $result = $this->decodeResult((string) $response->getBody());
            return new \WPCOM_VIP\Aws\Credentials\Credentials($result['AccessKeyId'], $result['SecretAccessKey'], $result['Token'], \strtotime($result['Expiration']));
        })->otherwise(function ($reason) {
            $reason = \is_array($reason) ? $reason['exception'] : $reason;
            $msg = $reason->getMessage();
            throw new \WPCOM_VIP\Aws\Exception\CredentialsException("Error retrieving credential from ECS ({$msg})");
        });
    }
    /**
    ...
    private function decodeResult($response)
    {
        ...
        if (!isset($result['AccessKeyId'])) {
            throw new \WPCOM_VIP\Aws\Exception\CredentialsException('Unexpected ECS credential value');
        }
        return $result;
    }
}
