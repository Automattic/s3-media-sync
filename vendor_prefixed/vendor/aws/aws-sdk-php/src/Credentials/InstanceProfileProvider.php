...
use WPCOM_VIP\Aws\Exception\InvalidJsonException;
use WPCOM_VIP\Aws\Sdk;
...
class InstanceProfileProvider
{
    ...
    public function __construct(array $config = [])
    {
        ...
        $this->attempts = 0;
        $this->client = isset($config['client']) ? $config['client'] : \WPCOM_VIP\Aws\default_http_handler();
    }
    /**
    ...
    public function __invoke()
    {
        return \WPCOM_VIP\GuzzleHttp\Promise\coroutine(function () {
            ...
            while ($this->secureMode && \is_null($token)) {
                ...
                } catch (\WPCOM_VIP\GuzzleHttp\Exception\RequestException $e) {
            ...
            while ($result == null) {
                ...
                } catch (\WPCOM_VIP\Aws\Exception\InvalidJsonException $e) {
            ...
            (yield new \WPCOM_VIP\Aws\Credentials\Credentials($result['AccessKeyId'], $result['SecretAccessKey'], $result['Token'], \strtotime($result['Expiration'])));
        });
    }
    /**
     * @param string $url
    ...
    private function request($url, $method = 'GET', $headers = [])
    {
        ...
        if (\strcasecmp($disabled, 'true') === 0) {
            throw new \WPCOM_VIP\Aws\Exception\CredentialsException($this->createErrorMessage('EC2 metadata service access disabled'));
        }
        $fn = $this->client;
        $request = new \WPCOM_VIP\GuzzleHttp\Psr7\Request($method, self::SERVER_URI . $url);
        $userAgent = 'aws-sdk-php/' . \WPCOM_VIP\Aws\Sdk::VERSION;
        if (\defined('HHVM_VERSION')) {
            $userAgent .= ' HHVM/' . HHVM_VERSION;
        }
        $userAgent .= ' ' . \WPCOM_VIP\Aws\default_user_agent();
        $request = $request->withHeader('User-Agent', $userAgent);
        foreach ($headers as $key => $value) {
            $request = $request->withHeader($key, $value);
        }
        return $fn($request, ['timeout' => $this->timeout])->then(function (\WPCOM_VIP\Psr\Http\Message\ResponseInterface $response) {
            return (string) $response->getBody();
        })->otherwise(function (array $reason) {
            $reason = $reason['exception'];
            if ($reason instanceof \WPCOM_VIP\GuzzleHttp\Exception\RequestException) {
                throw $reason;
            }
            $msg = $reason->getMessage();
            throw new \WPCOM_VIP\Aws\Exception\CredentialsException($this->createErrorMessage($msg));
        });
    }
    private function handleRetryableException(\Exception $e, $retryOptions, $message)
    {
        ...
        if ($isRetryable && $this->attempts < $this->retries) {
            ...
            throw new \WPCOM_VIP\Aws\Exception\CredentialsException($message);
        }
    }
    ...
    private function decodeResult($response)
    {
        ...
        if (\json_last_error() > 0) {
            throw new \WPCOM_VIP\Aws\Exception\InvalidJsonException();
        }
        if ($result['Code'] !== 'Success') {
            throw new \WPCOM_VIP\Aws\Exception\CredentialsException('Unexpected instance profile ' . 'response code: ' . $result['Code']);
        }
        return $result;
    }
}
