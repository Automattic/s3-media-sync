...
namespace WPCOM_VIP\Aws\Credentials;

use WPCOM_VIP\Aws;
use WPCOM_VIP\Aws\Api\DateTimeResult;
use WPCOM_VIP\Aws\CacheInterface;
use WPCOM_VIP\Aws\Exception\CredentialsException;
use WPCOM_VIP\Aws\Sts\StsClient;
...
class CredentialProvider
{
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (!empty(\getenv(\WPCOM_VIP\Aws\Credentials\EcsCredentialProvider::ENV_URI))) {
        ...
        if (isset($config['credentials']) && $config['credentials'] instanceof \WPCOM_VIP\Aws\CacheInterface) {
     ...
     * @return callable
     */
    public static function fromCredentials(\WPCOM_VIP\Aws\Credentials\CredentialsInterface $creds)
    {
        $promise = \WPCOM_VIP\GuzzleHttp\Promise\promise_for($creds);
    ...
    public static function memoize(callable $provider)
    {
        return function () use($provider) {
            ...
            return $result->then(function (\WPCOM_VIP\Aws\Credentials\CredentialsInterface $creds) use($provider, &$isConstant, &$result) {
                ...
                $result = null;
                return new \WPCOM_VIP\GuzzleHttp\Promise\RejectedPromise($reason);
            });
        };
    }
    /**
     ...
     * @return callable
     */
    public static function cache(callable $provider, \WPCOM_VIP\Aws\CacheInterface $cache, $cacheKey = null)
    {
        $cacheKey = $cacheKey ?: 'aws_cached_credentials';
        return function () use($provider, $cache, $cacheKey) {
            $found = $cache->get($cacheKey);
            if ($found instanceof \WPCOM_VIP\Aws\Credentials\CredentialsInterface && !$found->isExpired()) {
                return \WPCOM_VIP\GuzzleHttp\Promise\promise_for($found);
            }
            return $provider()->then(function (\WPCOM_VIP\Aws\Credentials\CredentialsInterface $creds) use($cache, $cacheKey) {
    ...
    public static function env()
    {
        return function () {
            ...
            if ($key && $secret) {
                return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\Credentials\Credentials($key, $secret, \getenv(self::ENV_SESSION) ?: NULL));
            }
    ...
    {
        return new \WPCOM_VIP\Aws\Credentials\InstanceProfileProvider($config);
    }
    /**
    ...
    {
        return new \WPCOM_VIP\Aws\Credentials\EcsCredentialProvider($config);
    }
    /**
    ...
    {
        return new \WPCOM_VIP\Aws\Credentials\AssumeRoleCredentialProvider($config);
    }
    /**
    ...
    public static function assumeRoleWithWebIdentityCredentialProvider(array $config = [])
    {
        return function () use($config) {
            ...
            if ($tokenFromEnv && $arnFromEnv) {
                ...
                $provider = new \WPCOM_VIP\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $arnFromEnv, 'WebIdentityTokenFile' => $tokenFromEnv, 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                return $provider();
            }
            ...
            if (isset($profiles[$profileName])) {
                ...
                if (isset($profile['web_identity_token_file']) && isset($profile['role_arn'])) {
                    ...
                    $provider = new \WPCOM_VIP\Aws\Credentials\AssumeRoleWithWebIdentityCredentialProvider(['RoleArn' => $profile['role_arn'], 'WebIdentityTokenFile' => $profile['web_identity_token_file'], 'SessionName' => $sessionName, 'client' => $stsClient, 'region' => $region]);
                    return $provider();
                }
            } else {
    ...
    public static function ini($profile = null, $filename = null, array $config = [])
    {
        ...
        return function () use($profile, $filename, $config) {
            ...
            */
            ...
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\Credentials\Credentials($data[$profile]['aws_access_key_id'], $data[$profile]['aws_secret_access_key'], $data[$profile]['aws_session_token']));
        };
    }
    /**
    ...
    public static function process($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            $data = \WPCOM_VIP\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
            if ($data === \false) {
                return self::reject("Invalid credentials file: {$filename}");
            }
            if (!isset($data[$profile])) {
            ...
            if (isset($processData['Expiration'])) {
                try {
                    $expiration = new \WPCOM_VIP\Aws\Api\DateTimeResult($processData['Expiration']);
                } catch (\Exception $e) {
                    return self::reject("credential_process returned invalid expiration");
                }
                $now = new \WPCOM_VIP\Aws\Api\DateTimeResult();
                if ($expiration < $now) {
            ...
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\Credentials\Credentials($processData['AccessKeyId'], $processData['SecretAccessKey'], $processData['SessionToken'], $expires));
        };
    }
    /**
    ...
    private static function loadRoleProfile($profiles, $profileName, $filename, $stsClient)
    {
        ...
        if (empty($stsClient)) {
            ...
            $sourceCredentials = \call_user_func(\WPCOM_VIP\Aws\Credentials\CredentialProvider::ini($sourceProfileName, $filename, $config))->wait();
            $stsClient = new \WPCOM_VIP\Aws\Sts\StsClient(['credentials' => $sourceCredentials, 'region' => $sourceRegion, 'version' => '2011-06-15']);
        }
        $result = $stsClient->assumeRole(['RoleArn' => $roleArn, 'RoleSessionName' => $roleSessionName]);
        $creds = $stsClient->createCredentials($result);
        return \WPCOM_VIP\GuzzleHttp\Promise\promise_for($creds);
    }
    /**
    ...
    {
        $profileData = \WPCOM_VIP\Aws\parse_ini_file($filename, \true, \INI_SCANNER_RAW);
        ...
        if ($filename === self::getHomeDir() . '/.aws/credentials' && \getenv('AWS_SDK_LOAD_NONDEFAULT_CONFIG')) {
            ...
            $configProfileData = \WPCOM_VIP\Aws\parse_ini_file($configFilename, \true, \INI_SCANNER_RAW);
    ...
    private static function loadDefaultProfiles()
    {
        ...
        if (\file_exists($credFile)) {
            $profiles = \WPCOM_VIP\Aws\parse_ini_file($credFile, \true, \INI_SCANNER_RAW);
        }
        if (\file_exists($configFile)) {
            $configProfileData = \WPCOM_VIP\Aws\parse_ini_file($configFile, \true, \INI_SCANNER_RAW);
            foreach ($configProfileData as $name => $profile) {
                // standardize config profile names
                $name = \str_replace('profile ', '', $name);
    ...
    {
        return new \WPCOM_VIP\GuzzleHttp\Promise\RejectedPromise(new \WPCOM_VIP\Aws\Exception\CredentialsException($msg));
    }
}
