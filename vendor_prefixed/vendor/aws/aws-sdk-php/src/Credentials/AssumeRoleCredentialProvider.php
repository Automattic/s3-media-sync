...
class AssumeRoleCredentialProvider
{
    ...
    {
        $client = $this->client;
        return $client->assumeRoleAsync($this->assumeRoleParams)->then(function (\WPCOM_VIP\Aws\Result $result) {
            return $this->client->createCredentials($result);
        })->otherwise(function (\RuntimeException $exception) {
            throw new \WPCOM_VIP\Aws\Exception\CredentialsException("Error in retrieving assume role credentials.", 0, $exception);
        });
    }
}
