...
namespace WPCOM_VIP\Aws\CloudFront;
...
class UrlSigner
{
    ...
    {
        $this->signer = new \WPCOM_VIP\Aws\CloudFront\Signer($keyPairId, $privateKey);
    }
    /**
    ...
    public function getSignedUrl($url, $expires = null, $policy = null)
    {
        ...
        $uri = new \WPCOM_VIP\GuzzleHttp\Psr7\Uri($scheme . '://' . $urlSections[1]);
        $query = \WPCOM_VIP\GuzzleHttp\Psr7\parse_query($uri->getQuery(), \PHP_QUERY_RFC3986);
    ...
    private function createRtmpUrl(\WPCOM_VIP\Psr\Http\Message\UriInterface $uri)
    {
