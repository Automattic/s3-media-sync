...
class RetryMiddleware
{
    ...
    public static function createDefaultDecider($maxRetries = 3, $extraConfig = [])
    {
        ...
        return function ($retries, \WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request, \WPCOM_VIP\Aws\ResultInterface $result = null, $error = null) use($maxRetries, $retryCurlErrors, $extraConfig) {
            ...
            if ($retries >= $maxRetries) {
                if (!empty($error) && $error instanceof \WPCOM_VIP\Aws\Exception\AwsException && $isRetryable) {
    ...
    private static function isRetryable($result, $error, $retryCurlErrors, $extraConfig = [])
    {
        ...
        if (!$error instanceof \WPCOM_VIP\Aws\Exception\AwsException) {
            return \false;
        }
        ...
        if (\count($retryCurlErrors) && ($previous = $error->getPrevious()) && $previous instanceof \WPCOM_VIP\GuzzleHttp\Exception\RequestException) {
    ...
    public function __invoke(\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request = null)
    {
        ...
        $g = function ($value) use($handler, $decider, $delay, $command, $request, &$retries, &$requestStats, &$monitoringEvents, &$g) {
            ...
            } elseif ($value instanceof \WPCOM_VIP\Aws\ResultInterface && !$decider($retries, $command, $request, $value, null)) {
