...
class TraceMiddleware
{
    ...
    public function __invoke($step, $name)
    {
        ...
        return function (callable $next) use($step, $name) {
            return function (\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request = null) use($next, $step, $name) {
    ...
    private function commandArray(\WPCOM_VIP\Aws\CommandInterface $cmd)
    {
        return ['instance' => \spl_object_hash($cmd), 'name' => $cmd->getName(), 'params' => $cmd->toArray()];
    }
    private function requestArray(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request = null)
    {
    ...
    private function responseArray(\WPCOM_VIP\Psr\Http\Message\ResponseInterface $response = null)
    {
    ...
    {
        return $value instanceof \WPCOM_VIP\Aws\ResultInterface ? ['instance' => \spl_object_hash($value), 'data' => $value->toArray()] : $value;
    }
    ...
    private function streamStr(\WPCOM_VIP\Psr\Http\Message\StreamInterface $body)
    {
        return $body->getSize() < $this->config['stream_size'] ? (string) $body : 'stream(size=' . $body->getSize() . ')';
    }
    private function createHttpDebug(\WPCOM_VIP\Aws\CommandInterface $command)
    {
        if ($this->config['http'] && !isset($command['@http']['debug'])) {
            $command['@http']['debug'] = \fopen('php://temp', 'w+');
        }
    }
    private function flushHttpDebug(\WPCOM_VIP\Aws\CommandInterface $command)
    {
