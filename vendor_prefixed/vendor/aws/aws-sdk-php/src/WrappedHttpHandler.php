...
class WrappedHttpHandler
{
     ...
     *                                 information.
     */
    public function __construct(callable $httpHandler, callable $parser, callable $errorParser, $exceptionClass = 'WPCOM_VIP\\Aws\\Exception\\AwsException', $collectStats = \false)
    {
    ...
    public function __invoke(\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request)
    {
        ...
        return \WPCOM_VIP\GuzzleHttp\Promise\promise_for($fn($request, $options))->then(function (\WPCOM_VIP\Psr\Http\Message\ResponseInterface $res) use($command, $request, &$stats) {
            ...
            return new \WPCOM_VIP\GuzzleHttp\Promise\RejectedPromise($err);
        });
    }
    /**
     ...
     */
    private function parseResponse(\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request, \WPCOM_VIP\Psr\Http\Message\ResponseInterface $response, array $stats)
    {
        $parser = $this->parser;
        $status = $response->getStatusCode();
        $result = $status < 300 ? $parser($command, $response) : new \WPCOM_VIP\Aws\Result();
     ...
     * @return \Exception
     */
    private function parseError(array $err, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request, \WPCOM_VIP\Aws\CommandInterface $command, array $stats)
    {
