...
trait EncryptionTraitV2
{
    ...
    private static $encryptClasses = ['gcm' => \WPCOM_VIP\Aws\Crypto\AesGcmEncryptingStream::class];
    /**
     ...
     * @internal
     */
    public function encrypt(\WPCOM_VIP\GuzzleHttp\Psr7\Stream $plaintext, array $options, \WPCOM_VIP\Aws\Crypto\MaterialsProviderV2 $provider, \WPCOM_VIP\Aws\Crypto\MetadataEnvelope $envelope)
    {
        ...
        if (!\WPCOM_VIP\Aws\Crypto\MaterialsProviderV2::isSupportedKeySize($cipherOptions['KeySize'])) {
        ...
        $envelope[\WPCOM_VIP\Aws\Crypto\MetadataEnvelope::CONTENT_KEY_V2_HEADER] = $keys['Ciphertext'];
        unset($keys);
