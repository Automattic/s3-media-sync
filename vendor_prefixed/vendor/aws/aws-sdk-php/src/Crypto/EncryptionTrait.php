...
use WPCOM_VIP\GuzzleHttp\Psr7\AppendStream;
...
trait EncryptionTrait
{
     ...
     * @internal
     */
    public function encrypt(\WPCOM_VIP\GuzzleHttp\Psr7\Stream $plaintext, array $cipherOptions, \WPCOM_VIP\Aws\Crypto\MaterialsProvider $provider, \WPCOM_VIP\Aws\Crypto\MetadataEnvelope $envelope)
    {
        ...
        if (!\WPCOM_VIP\Aws\Crypto\MaterialsProvider::isSupportedKeySize($cipherOptions['KeySize'])) {
        ...
        $envelope[\WPCOM_VIP\Aws\Crypto\MetadataEnvelope::CONTENT_KEY_V2_HEADER] = $provider->encryptCek($cek, $materialsDescription);
        unset($cek);
        $envelope[\WPCOM_VIP\Aws\Crypto\MetadataEnvelope::IV_HEADER] = \base64_encode($cipherOptions['Iv']);
        $envelope[\WPCOM_VIP\Aws\Crypto\MetadataEnvelope::KEY_WRAP_ALGORITHM_HEADER] = $provider->getWrapAlgorithmName();
        $envelope[\WPCOM_VIP\Aws\Crypto\MetadataEnvelope::CONTENT_CRYPTO_SCHEME_HEADER] = $aesName;
        $envelope[\WPCOM_VIP\Aws\Crypto\MetadataEnvelope::UNENCRYPTED_CONTENT_LENGTH_HEADER] = \strlen($plaintext);
        $envelope[\WPCOM_VIP\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER] = \json_encode($materialsDescription);
        if (!empty($cipherOptions['Tag'])) {
            $envelope[\WPCOM_VIP\Aws\Crypto\MetadataEnvelope::CRYPTO_TAG_LENGTH_HEADER] = \strlen($cipherOptions['Tag']) * 8;
        }
     ...
     * @internal
     */
    protected function getEncryptingStream(\WPCOM_VIP\GuzzleHttp\Psr7\Stream $plaintext, $cek, &$cipherOptions)
    {
        switch ($cipherOptions['Cipher']) {
            case 'gcm':
                $cipherOptions['TagLength'] = 16;
                $cipherTextStream = new \WPCOM_VIP\Aws\Crypto\AesGcmEncryptingStream($plaintext, $cek, $cipherOptions['Iv'], $cipherOptions['Aad'] = isset($cipherOptions['Aad']) ? $cipherOptions['Aad'] : null, $cipherOptions['TagLength'], $cipherOptions['KeySize']);
                ...
                $appendStream = new \WPCOM_VIP\GuzzleHttp\Psr7\AppendStream([$cipherTextStream->createStream()]);
                $cipherOptions['Tag'] = $cipherTextStream->getTag();
                $appendStream->addStream(\WPCOM_VIP\GuzzleHttp\Psr7\stream_for($cipherOptions['Tag']));
            ...
            default:
                ...
                $cipherTextStream = new \WPCOM_VIP\Aws\Crypto\AesEncryptingStream($plaintext, $cek, $cipherMethod);
