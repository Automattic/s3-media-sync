 ...
 * @deprecated
 */
class KmsMaterialsProvider extends \WPCOM_VIP\Aws\Crypto\MaterialsProvider implements \WPCOM_VIP\Aws\Crypto\MaterialsProviderInterface
{
    ...
    public function fromDecryptionEnvelope(\WPCOM_VIP\Aws\Crypto\MetadataEnvelope $envelope)
    {
        if (empty($envelope[\WPCOM_VIP\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER])) {
            throw new \RuntimeException('Not able to detect the materials description.');
        }
        $materialsDescription = \json_decode($envelope[\WPCOM_VIP\Aws\Crypto\MetadataEnvelope::MATERIALS_DESCRIPTION_HEADER], \true);
