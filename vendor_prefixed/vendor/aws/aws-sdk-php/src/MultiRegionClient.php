...
use WPCOM_VIP\Aws\Endpoint\PartitionInterface;
class MultiRegionClient implements \WPCOM_VIP\Aws\AwsClientInterface
{
    use AwsClientTrait;
    ...
    {
        $args = \array_intersect_key(\WPCOM_VIP\Aws\ClientResolver::getDefaultArguments(), ['service' => \true, 'region' => \true]);
        ...
        }], 'partition' => ['type' => 'config', 'valid' => ['string', \WPCOM_VIP\Aws\Endpoint\PartitionInterface::class], 'doc' => 'AWS partition to connect to. Valid partitions' . ' include "aws," "aws-cn," and "aws-us-gov." Used to' . ' restrict the scope of the mapRegions method.', 'default' => function (array $args) {
            $region = isset($args['region']) ? $args['region'] : '';
            return \WPCOM_VIP\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartition($region, $args['service']);
        }, 'fn' => function ($value, array &$args) {
            if (\is_string($value)) {
                $value = \WPCOM_VIP\Aws\Endpoint\PartitionEndpointProvider::defaultProvider()->getPartitionByName($value);
            }
            if (!$value instanceof \WPCOM_VIP\Aws\Endpoint\PartitionInterface) {
    ...
    public function __construct(array $args = [])
    {
        ...
        $this->handlerList = new \WPCOM_VIP\Aws\HandlerList(function (\WPCOM_VIP\Aws\CommandInterface $command) {
        ...
        $resolver = new \WPCOM_VIP\Aws\ClientResolver($argDefinitions);
