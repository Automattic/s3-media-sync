...
use WPCOM_VIP\Aws\Api\Validator;
use WPCOM_VIP\Aws\Api\ApiProvider;
use WPCOM_VIP\Aws\Api\Service;
use WPCOM_VIP\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware;
use WPCOM_VIP\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware;
use WPCOM_VIP\Aws\ClientSideMonitoring\Configuration;
use WPCOM_VIP\Aws\Credentials\Credentials;
use WPCOM_VIP\Aws\Credentials\CredentialsInterface;
use WPCOM_VIP\Aws\Endpoint\PartitionEndpointProvider;
use WPCOM_VIP\Aws\EndpointDiscovery\ConfigurationInterface;
use WPCOM_VIP\Aws\EndpointDiscovery\ConfigurationProvider;
use WPCOM_VIP\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware;
use WPCOM_VIP\Aws\Exception\InvalidRegionException;
use WPCOM_VIP\Aws\Retry\ConfigurationInterface as RetryConfigInterface;
use WPCOM_VIP\Aws\Retry\ConfigurationProvider as RetryConfigProvider;
use WPCOM_VIP\Aws\Signature\SignatureProvider;
use WPCOM_VIP\Aws\Endpoint\EndpointProvider;
use WPCOM_VIP\Aws\Credentials\CredentialProvider;
...
class ClientResolver
{
     ...
     */
    public function resolve(array $args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        $args['config'] = [];
    ...
    public static function _apply_retries($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        // A value of 0 for the config option disables retries
        if ($value) {
            $config = \WPCOM_VIP\Aws\Retry\ConfigurationProvider::unwrap($value);
            if ($config->getMode() === 'legacy') {
                // # of retries is 1 less than # of attempts
                $decider = \WPCOM_VIP\Aws\RetryMiddleware::createDefaultDecider($config->getMaxAttempts() - 1);
                $list->appendSign(\WPCOM_VIP\Aws\Middleware::retry($decider, null, $args['stats']['retries']), 'retry');
            } else {
                $list->appendSign(\WPCOM_VIP\Aws\RetryMiddlewareV2::wrap($config, ['collect_stats' => $args['stats']['retries']]), 'retry');
            }
        }
    }
    public static function _apply_credentials($value, array &$args)
    {
        ...
        if ($value instanceof \WPCOM_VIP\Aws\Credentials\CredentialsInterface) {
            $args['credentials'] = \WPCOM_VIP\Aws\Credentials\CredentialProvider::fromCredentials($value);
        } elseif (\is_array($value) && isset($value['key']) && isset($value['secret'])) {
            $args['credentials'] = \WPCOM_VIP\Aws\Credentials\CredentialProvider::fromCredentials(new \WPCOM_VIP\Aws\Credentials\Credentials($value['key'], $value['secret'], isset($value['token']) ? $value['token'] : null, isset($value['expires']) ? $value['expires'] : null));
        } elseif ($value === \false) {
            $args['credentials'] = \WPCOM_VIP\Aws\Credentials\CredentialProvider::fromCredentials(new \WPCOM_VIP\Aws\Credentials\Credentials('', ''));
            $args['config']['signature_version'] = 'anonymous';
        } elseif ($value instanceof \WPCOM_VIP\Aws\CacheInterface) {
            $args['credentials'] = \WPCOM_VIP\Aws\Credentials\CredentialProvider::defaultProvider($args);
        } else {
    ...
    {
        return \WPCOM_VIP\Aws\Credentials\CredentialProvider::defaultProvider($args);
    }
    public static function _apply_csm($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        if ($value === \false) {
            $value = new \WPCOM_VIP\Aws\ClientSideMonitoring\Configuration(\false, \WPCOM_VIP\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \WPCOM_VIP\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \WPCOM_VIP\Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
        $list->appendBuild(\WPCOM_VIP\Aws\ClientSideMonitoring\ApiCallMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallMonitoringMiddleware');
        $list->appendAttempt(\WPCOM_VIP\Aws\ClientSideMonitoring\ApiCallAttemptMonitoringMiddleware::wrap($args['credentials'], $value, $args['region'], $args['api']->getServiceId()), 'ApiCallAttemptMonitoringMiddleware');
    }
    public static function _apply_api_provider(callable $value, array &$args)
    {
        $api = new \WPCOM_VIP\Aws\Api\Service(\WPCOM_VIP\Aws\Api\ApiProvider::resolve($value, 'api', $args['service'], $args['version']), $value);
        if (empty($args['config']['signing_name']) && isset($api['metadata']['signingName'])) {
            $args['config']['signing_name'] = $api['metadata']['signingName'];
        }
        $args['api'] = $api;
        $args['parser'] = \WPCOM_VIP\Aws\Api\Service::createParser($api);
        $args['error_parser'] = \WPCOM_VIP\Aws\Api\Service::createErrorParser($api->getProtocol(), $api);
    }
    public static function _apply_endpoint_provider(callable $value, array &$args)
    {
        if (!isset($args['endpoint'])) {
            ...
            if (!self::isValidRegion($args['region'])) {
                throw new \WPCOM_VIP\Aws\Exception\InvalidRegionException('Region must be a valid RFC' . ' host label.');
            }
            // Invoke the endpoint provider and throw if it does not resolve.
            $result = \WPCOM_VIP\Aws\Endpoint\EndpointProvider::resolve($value, ['service' => $endpointPrefix, 'region' => $args['region'], 'scheme' => $args['scheme'], 'options' => self::getEndpointProviderOptions($args)]);
    ...
    {
        return \WPCOM_VIP\Aws\EndpointDiscovery\ConfigurationProvider::defaultProvider($args);
    }
    public static function _apply_serializer($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        $list->prependBuild(\WPCOM_VIP\Aws\Middleware::requestBuilder($value), 'builder');
    }
    public static function _apply_debug($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        if ($value !== \false) {
            $list->interpose(new \WPCOM_VIP\Aws\TraceMiddleware($value === \true ? [] : $value));
        }
    }
    public static function _apply_stats($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        ...
        if ($args['stats']['timer']) {
            $list->prependInit(\WPCOM_VIP\Aws\Middleware::timer(), 'timer');
        }
    }
    public static function _apply_profile($_, array &$args)
    {
        $args['credentials'] = \WPCOM_VIP\Aws\Credentials\CredentialProvider::ini($args['profile']);
    }
    public static function _apply_validate($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        if ($value === \false) {
            return;
        }
        $validator = $value === \true ? new \WPCOM_VIP\Aws\Api\Validator() : new \WPCOM_VIP\Aws\Api\Validator($value);
        $list->appendValidate(\WPCOM_VIP\Aws\Middleware::validation($args['api'], $validator), 'validation');
    }
    public static function _apply_handler($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        $list->setHandler($value);
    }
    public static function _default_handler(array &$args)
    {
        return new \WPCOM_VIP\Aws\WrappedHttpHandler(default_http_handler(), $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_http_handler($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        $args['handler'] = new \WPCOM_VIP\Aws\WrappedHttpHandler($value, $args['parser'], $args['error_parser'], $args['exception_class'], $args['stats']['http']);
    }
    public static function _apply_user_agent($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        if (!\is_array($value)) {
            $value = [$value];
        }
        ...
        \array_unshift($value, 'aws-sdk-php/' . \WPCOM_VIP\Aws\Sdk::VERSION);
        $args['ua_append'] = $value;
        $list->appendBuild(static function (callable $handler) use($value) {
            return function (\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request) use($handler, $value) {
                return $handler($command, $request->withHeader('User-Agent', \implode(' ', \array_merge($value, $request->getHeader('User-Agent')))));
            };
        });
    }
    public static function _apply_endpoint($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
    ...
    public static function _apply_idempotency_auto_fill($value, array &$args, \WPCOM_VIP\Aws\HandlerList $list)
    {
        $enabled = \false;
        $generator = null;
        if (\is_bool($value)) {
        ...
        if ($enabled) {
            $list->prependInit(\WPCOM_VIP\Aws\IdempotencyTokenMiddleware::wrap($args['api'], $generator), 'idempotency_auto_fill');
        }
    }
    public static function _default_endpoint_provider(array $args)
    {
        $options = self::getEndpointProviderOptions($args);
        return \WPCOM_VIP\Aws\Endpoint\PartitionEndpointProvider::defaultProvider($options)->getPartition($args['region'], $args['service']);
    }
    public static function _default_serializer(array $args)
    {
        return \WPCOM_VIP\Aws\Api\Service::createSerializer($args['api'], $args['endpoint']);
    }
    public static function _default_signature_provider()
    {
        return \WPCOM_VIP\Aws\Signature\SignatureProvider::defaultProvider();
    }
    public static function _default_signature_version(array &$args)
    {
        ...
        $args['__partition_result'] = isset($args['__partition_result']) ? isset($args['__partition_result']) : \call_user_func(\WPCOM_VIP\Aws\Endpoint\PartitionEndpointProvider::defaultProvider(), ['service' => $args['service'], 'region' => $args['region']]);
    ...
    public static function _missing_version(array $args)
    {
        ...
        $versions = \WPCOM_VIP\Aws\Api\ApiProvider::defaultProvider()->getVersions($service);
