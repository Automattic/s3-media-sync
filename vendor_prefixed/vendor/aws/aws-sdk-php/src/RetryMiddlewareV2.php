...
use WPCOM_VIP\Aws\Retry\ConfigurationInterface;
use WPCOM_VIP\Aws\Retry\QuotaManager;
use WPCOM_VIP\Aws\Retry\RateLimiter;
use WPCOM_VIP\Aws\Retry\RetryHelperTrait;
...
class RetryMiddlewareV2
{
    ...
    public static function createDefaultDecider(\WPCOM_VIP\Aws\Retry\QuotaManager $quotaManager, $maxAttempts = 3, $options = [])
    {
        $retryCurlErrors = [];
        if (\extension_loaded('curl')) {
            $retryCurlErrors[\CURLE_RECV_ERROR] = \true;
        }
        return function ($attempts, \WPCOM_VIP\Aws\CommandInterface $command, $result) use($options, $quotaManager, $retryCurlErrors, $maxAttempts) {
            ...
            if ($isRetryable) {
                ...
                if ($attempts >= $maxAttempts) {
                    if (!empty($result) && $result instanceof \WPCOM_VIP\Aws\Exception\AwsException) {
    ...
    public function __construct(\WPCOM_VIP\Aws\Retry\ConfigurationInterface $config, callable $handler, $options = [])
    {
        $this->options = $options;
        $this->maxAttempts = $config->getMaxAttempts();
        $this->mode = $config->getMode();
        $this->nextHandler = $handler;
        $this->quotaManager = new \WPCOM_VIP\Aws\Retry\QuotaManager();
        ...
        if ($this->mode === 'adaptive') {
            $this->rateLimiter = isset($options['rate_limiter']) ? $options['rate_limiter'] : new \WPCOM_VIP\Aws\Retry\RateLimiter();
        }
    }
    public function __invoke(\WPCOM_VIP\Aws\CommandInterface $cmd, \WPCOM_VIP\Psr\Http\Message\RequestInterface $req)
    {
        ...
        $callback = function ($value) use($handler, $cmd, $req, $decider, $delayer, &$attempts, &$requestStats, &$monitoringEvents, &$callback) {
            ...
            if ($value instanceof \WPCOM_VIP\Aws\MonitoringEventsInterface) {
            ...
            if ($value instanceof \Exception || $value instanceof \Throwable) {
                if (!$decider($attempts, $cmd, $value)) {
                    return \WPCOM_VIP\GuzzleHttp\Promise\rejection_for($this->bindStatsToReturn($value, $requestStats));
                }
            } elseif ($value instanceof \WPCOM_VIP\Aws\ResultInterface && !$decider($attempts, $cmd, $value)) {
    ...
    private static function isRetryable($result, $retryCurlErrors, $options = [])
    {
        ...
        if (!$result instanceof \WPCOM_VIP\Aws\Exception\AwsException) {
            return \false;
        }
        ...
        if (\count($retryCurlErrors) && ($previous = $result->getPrevious()) && $previous instanceof \WPCOM_VIP\GuzzleHttp\Exception\RequestException) {
