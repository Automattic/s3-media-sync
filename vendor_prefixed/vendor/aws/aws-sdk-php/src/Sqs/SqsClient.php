...
namespace WPCOM_VIP\Aws\Sqs;

use WPCOM_VIP\Aws\AwsClient;
use WPCOM_VIP\Aws\CommandInterface;
use WPCOM_VIP\Aws\Sqs\Exception\SqsException;
 ...
 */
class SqsClient extends \WPCOM_VIP\Aws\AwsClient
{
        ...
        return static function (callable $handler) {
            return function (\WPCOM_VIP\Aws\CommandInterface $c, \WPCOM_VIP\Psr\Http\Message\RequestInterface $r = null) use($handler) {
                if ($c->hasParam('QueueUrl')) {
                    $r = $r->withUri(\WPCOM_VIP\GuzzleHttp\Psr7\UriResolver::resolve($r->getUri(), new \WPCOM_VIP\GuzzleHttp\Psr7\Uri($c['QueueUrl'])));
                }
                return $handler($c, $r);
            };
        };
                ...
                return $handler($c, $r)->then(function ($result) use($c, $r) {
                    foreach ((array) $result['Messages'] as $msg) {
                        ...
                        if (isset($msg['MD5OfBody']) && $bodyMd5 !== $msg['MD5OfBody']) {
                            throw new \WPCOM_VIP\Aws\Sqs\Exception\SqsException(\sprintf('MD5 mismatch. Expected %s, found %s', $msg['MD5OfBody'], $bodyMd5), $c, ['code' => 'ClientChecksumMismatch', 'request' => $r]);
                        }
                        if (isset($msg['MD5OfMessageAttributes'])) {
                            ...
                            if ($messageAttributesMd5 !== $msg['MD5OfMessageAttributes']) {
                                throw new \WPCOM_VIP\Aws\Sqs\Exception\SqsException(\sprintf('Attribute MD5 mismatch. Expected %s, found %s', $msg['MD5OfMessageAttributes'], $messageAttributesMd5 ? $messageAttributesMd5 : 'No Attributes'), $c, ['code' => 'ClientChecksumMismatch', 'request' => $r]);
                            }
                        } else {
                            if (isset($msg['MessageAttributes'])) {
                                throw new \WPCOM_VIP\Aws\Sqs\Exception\SqsException(\sprintf('No Attribute MD5 found. Expected %s', self::calculateMessageAttributesMd5($msg)), $c, ['code' => 'ClientChecksumMismatch', 'request' => $r]);
                            }
                        }
                    }
                    return $result;
