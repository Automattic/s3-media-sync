...
namespace WPCOM_VIP\Aws\MachineLearning;
 ...
 */
class MachineLearningClient extends \WPCOM_VIP\Aws\AwsClient
{
        ...
        return static function (callable $handler) {
            return function (\WPCOM_VIP\Aws\CommandInterface $command, \WPCOM_VIP\Psr\Http\Message\RequestInterface $request = null) use($handler) {
                if ($command->getName() === 'Predict') {
                    $request = $request->withUri(new \WPCOM_VIP\GuzzleHttp\Psr7\Uri($command['PredictEndpoint']));
                }
