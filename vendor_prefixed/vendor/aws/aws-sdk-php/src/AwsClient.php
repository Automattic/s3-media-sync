...
use WPCOM_VIP\Aws\ClientSideMonitoring\ConfigurationProvider;
 ...
 */
class AwsClient implements \WPCOM_VIP\Aws\AwsClientInterface
{
    use AwsClientTrait;
    /** @var array */
    private $aliases;
    ...
    {
        return \WPCOM_VIP\Aws\ClientResolver::getDefaultArguments();
    }
    /**
    ...
    public function __construct(array $args)
    {
        ...
        $this->handlerList = new \WPCOM_VIP\Aws\HandlerList();
        $resolver = new \WPCOM_VIP\Aws\ClientResolver(static::getArguments());
        $config = $resolver->resolve($args, $this->handlerList);
        $this->api = $config['api'];
        $this->signatureProvider = $config['signature_provider'];
        $this->endpoint = new \WPCOM_VIP\GuzzleHttp\Psr7\Uri($config['endpoint']);
    ...
    public function getCommand($name, array $args = [])
    {
        ...
        return new \WPCOM_VIP\Aws\Command($name, $args, clone $this->getHandlerList());
    }
    ...
    private function parseClass()
    {
        ...
        if ($klass === __CLASS__) {
            return ['', 'WPCOM_VIP\\Aws\\Exception\\AwsException'];
        }
    ...
    private function addEndpointParameterMiddleware($args)
    {
        if (empty($args['disable_host_prefix_injection'])) {
            ...
            $list->appendBuild(\WPCOM_VIP\Aws\EndpointParameterMiddleware::wrap($this->api), 'endpoint_parameter');
        }
    }
    private function addEndpointDiscoveryMiddleware($config, $args)
    {
        ...
        if (!isset($args['endpoint'])) {
            $list->appendBuild(\WPCOM_VIP\Aws\EndpointDiscovery\EndpointDiscoveryMiddleware::wrap($this, $args, $config['endpoint_discovery']), 'EndpointDiscoveryMiddleware');
        }
    }
    private function addSignatureMiddleware()
    {
        ...
        $resolver = static function (\WPCOM_VIP\Aws\CommandInterface $c) use($api, $provider, $name, $region, $version) {
            ...
            return \WPCOM_VIP\Aws\Signature\SignatureProvider::resolve($provider, $version, $name, $region);
        };
        $this->handlerList->appendSign(\WPCOM_VIP\Aws\Middleware::signer($this->credentialProvider, $resolver), 'signer');
    }
    private function addInvocationId()
    {
        // Add invocation id to each request
        $this->handlerList->prependSign(\WPCOM_VIP\Aws\Middleware::invocationId(), 'invocation-id');
    }
    private function loadAliases($file = null)
    {
        if (!isset($this->aliases)) {
            ...
            $aliases = \WPCOM_VIP\Aws\load_compiled_json($file);
    ...
    {
        $streamRequestPayloadMiddleware = \WPCOM_VIP\Aws\StreamRequestPayloadMiddleware::wrap($this->api);
    ...
    {
        $aliases = \WPCOM_VIP\Aws\load_compiled_json(__DIR__ . '/data/aliases.json');
