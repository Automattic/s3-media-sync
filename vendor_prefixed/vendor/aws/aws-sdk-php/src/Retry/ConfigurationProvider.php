...
use WPCOM_VIP\Aws\Retry\Exception\ConfigurationException;
...
class ConfigurationProvider extends \WPCOM_VIP\Aws\AbstractConfigurationProvider implements \WPCOM_VIP\Aws\ConfigurationProviderInterface
{
    ...
    protected static $interfaceClass = \WPCOM_VIP\Aws\Retry\ConfigurationInterface::class;
    protected static $exceptionClass = \WPCOM_VIP\Aws\Retry\Exception\ConfigurationException::class;
    /**
    ...
    public static function defaultProvider(array $config = [])
    {
        ...
        if (isset($config['retries']) && $config['retries'] instanceof \WPCOM_VIP\Aws\CacheInterface) {
    ...
    public static function env()
    {
        return function () {
            ...
            if (!empty($mode)) {
                return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\Retry\Configuration($mode, $maxAttempts));
            }
    ...
    {
        return function () {
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\Retry\Configuration(self::DEFAULT_MODE, self::DEFAULT_MAX_ATTEMPTS));
        };
    }
    /**
    ...
    public static function ini($profile = null, $filename = null)
    {
        ...
        return function () use($profile, $filename) {
            ...
            return \WPCOM_VIP\GuzzleHttp\Promise\promise_for(new \WPCOM_VIP\Aws\Retry\Configuration($data[$profile][self::INI_MODE], $maxAttempts));
        };
    }
    /**
    ...
    public static function unwrap($config)
    {
        ...
        if ($config instanceof \WPCOM_VIP\Aws\Retry\ConfigurationInterface) {
            return $config;
        }
        ...
        if (\is_int($config)) {
            return new \WPCOM_VIP\Aws\Retry\Configuration('legacy', $config + 1);
        }
        if (\is_array($config) && isset($config['mode'])) {
            $maxAttempts = isset($config['max_attempts']) ? $config['max_attempts'] : self::DEFAULT_MAX_ATTEMPTS;
            return new \WPCOM_VIP\Aws\Retry\Configuration($config['mode'], $maxAttempts);
        }
