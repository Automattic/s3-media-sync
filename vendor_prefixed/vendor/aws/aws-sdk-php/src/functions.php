...
function default_http_handler()
{
    ...
    if ($version === 6 || $version === 7) {
        return new \WPCOM_VIP\Aws\Handler\GuzzleV6\GuzzleHandler();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return new \WPCOM_VIP\Aws\Handler\GuzzleV5\GuzzleHandler();
    }
...
function default_user_agent()
{
    ...
    // If Guzzle 6 or 7 installed
    if ($version === 6 || $version === 7) {
        return \WPCOM_VIP\GuzzleHttp\default_user_agent();
    }
    // If Guzzle 5 installed
    if ($version === 5) {
        return \WPCOM_VIP\GuzzleHttp\Client::getDefaultUserAgent();
    }
    throw new \RuntimeException('Unknown Guzzle version: ' . $version);
}
/**
...
function guzzle_major_version()
{
    ...
    if (\defined('\\WPCOM_VIP\\GuzzleHttp\\ClientInterface::VERSION')) {
        $version = (string) \WPCOM_VIP\GuzzleHttp\ClientInterface::VERSION;
        if ($version[0] === '6') {
            return $cache = 6;
        }
        if ($version[0] === '5') {
    ...
    } elseif (\defined('\\WPCOM_VIP\\GuzzleHttp\\ClientInterface::MAJOR_VERSION')) {
        return $cache = \WPCOM_VIP\GuzzleHttp\ClientInterface::MAJOR_VERSION;
    }
 ...
 * @throws \RuntimeException
 */
function serialize(\WPCOM_VIP\Aws\CommandInterface $command)
{
    $request = null;
    $handlerList = $command->getHandlerList();
    // Return a mock result.
    $handlerList->setHandler(function (\WPCOM_VIP\Aws\CommandInterface $_, \WPCOM_VIP\Psr\Http\Message\RequestInterface $r) use(&$request) {
        $request = $r;
        return new \WPCOM_VIP\GuzzleHttp\Promise\FulfilledPromise(new \WPCOM_VIP\Aws\Result([]));
    });
    \call_user_func($handlerList->resolve(), $command)->wait();
    if (!$request instanceof \WPCOM_VIP\Psr\Http\Message\RequestInterface) {
