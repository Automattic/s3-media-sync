...
class StreamHandler
{
    ...
    public function __invoke(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options) : \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        $startTime = isset($options['on_stats']) ? \WPCOM_VIP\GuzzleHttp\Utils::currentTime() : null;
        try {
            ...
            if (\false !== \strpos($message, 'getaddrinfo') || \false !== \strpos($message, 'Connection refused') || \false !== \strpos($message, "couldn't connect to host") || \false !== \strpos($message, "connection attempt failed")) {
                $e = new \WPCOM_VIP\GuzzleHttp\Exception\ConnectException($e->getMessage(), $request, $e);
            } else {
     ...
     */
    private function createResponse(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options, $stream, ?float $startTime) : \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        $response = new \WPCOM_VIP\GuzzleHttp\Psr7\Response($status, $headers, $sink, $ver, $reason);
        ...
        return new \WPCOM_VIP\GuzzleHttp\Promise\FulfilledPromise($response);
    }
    private function createSink(\WPCOM_VIP\Psr\Http\Message\StreamInterface $stream, array $options) : \WPCOM_VIP\Psr\Http\Message\StreamInterface
    {
    ...
    private function checkDecode(array $options, array $headers, $stream) : array
    {
        ...
        if (!empty($options['decode_content'])) {
            $normalizedKeys = \WPCOM_VIP\GuzzleHttp\Utils::normalizeHeaderKeys($headers);
     ...
     */
    private function drain(\WPCOM_VIP\Psr\Http\Message\StreamInterface $source, \WPCOM_VIP\Psr\Http\Message\StreamInterface $sink, string $contentLength) : \WPCOM_VIP\Psr\Http\Message\StreamInterface
    {
     ...
     * @return resource
     */
    private function createStream(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options)
    {
        static $methods;
        if (!$methods) {
        ...
        return $this->createResource(function () use($uri, &$http_response_header, $contextResource, $context, $options, $request) {
            ...
            if (\false === $resource) {
                throw new \WPCOM_VIP\GuzzleHttp\Exception\ConnectException(\sprintf('Connection refused for URI %s', $uri), $request, null, $context);
            }
    ...
    private function resolveHost(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options) : \WPCOM_VIP\Psr\Http\Message\UriInterface
    {
        $uri = $request->getUri();
        if (isset($options['force_ip_resolve']) && !\filter_var($uri->getHost(), \FILTER_VALIDATE_IP)) {
            if ('v4' === $options['force_ip_resolve']) {
                ...
                if (\false === $records || !isset($records[0]['ip'])) {
                    throw new \WPCOM_VIP\GuzzleHttp\Exception\ConnectException(\sprintf("Could not resolve IPv4 address for host '%s'", $uri->getHost()), $request);
                }
            ...
            if ('v6' === $options['force_ip_resolve']) {
                ...
                if (\false === $records || !isset($records[0]['ipv6'])) {
                    throw new \WPCOM_VIP\GuzzleHttp\Exception\ConnectException(\sprintf("Could not resolve IPv6 address for host '%s'", $uri->getHost()), $request);
                }
    ...
    private function getDefaultContext(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request) : array
    {
        $headers = '';
     ...
     */
    private function add_proxy(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array &$options, $value, array &$params) : void
    {
        if (!\is_array($value)) {
            ...
            if (isset($value[$scheme])) {
                if (!isset($value['no']) || !\WPCOM_VIP\GuzzleHttp\Utils::isHostInNoProxy($request->getUri()->getHost(), $value['no'])) {
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_timeout(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array &$options, $value, array &$params) : void
    {
        if ($value > 0) {
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_verify(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array &$options, $value, array &$params) : void
    {
        if ($value === \false) {
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_cert(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array &$options, $value, array &$params) : void
    {
        if (\is_array($value)) {
     ...
     * @param mixed $value as passed via Request transfer options.
     */
    private function add_debug(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array &$options, $value, array &$params) : void
    {
        if ($value === \false) {
            return;
        }
