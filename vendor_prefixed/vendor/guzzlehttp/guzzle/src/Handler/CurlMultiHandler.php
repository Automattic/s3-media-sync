...
class CurlMultiHandler
{
    ...
    {
        $this->factory = $options['handle_factory'] ?? new \WPCOM_VIP\GuzzleHttp\Handler\CurlFactory(50);
    ...
    public function __invoke(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options) : \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        $id = (int) $easy->handle;
        $promise = new \WPCOM_VIP\GuzzleHttp\Promise\Promise([$this, 'execute'], function () use($id) {
    ...
    public function tick() : void
    {
        ...
        if ($this->delays) {
            $currentTime = \WPCOM_VIP\GuzzleHttp\Utils::currentTime();
    ...
    private function addRequest(array $entry) : void
    {
        ...
        if (empty($easy->options['delay'])) {
            ...
            $this->delays[$id] = \WPCOM_VIP\GuzzleHttp\Utils::currentTime() + $easy->options['delay'] / 1000;
        }
    }
    /**
    ...
    private function processMessages() : void
    {
        while ($done = \curl_multi_info_read($this->_mh)) {
            ...
            $entry['deferred']->resolve(\WPCOM_VIP\GuzzleHttp\Handler\CurlFactory::finish($this, $entry['easy'], $this->factory));
        }
    }
