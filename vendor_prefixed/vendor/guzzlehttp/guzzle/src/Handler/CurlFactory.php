...
class CurlFactory implements \WPCOM_VIP\GuzzleHttp\Handler\CurlFactoryInterface
{
    ...
    public function create(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options) : \WPCOM_VIP\GuzzleHttp\Handler\EasyHandle
    {
        ...
        $easy = new \WPCOM_VIP\GuzzleHttp\Handler\EasyHandle();
     ...
     */
    public static function finish(callable $handler, \WPCOM_VIP\GuzzleHttp\Handler\EasyHandle $easy, \WPCOM_VIP\GuzzleHttp\Handler\CurlFactoryInterface $factory) : \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        return new \WPCOM_VIP\GuzzleHttp\Promise\FulfilledPromise($easy->response);
    }
    private static function invokeStats(\WPCOM_VIP\GuzzleHttp\Handler\EasyHandle $easy) : void
    {
     ...
     * @param callable(RequestInterface, array): PromiseInterface $handler
     */
    private static function finishError(callable $handler, \WPCOM_VIP\GuzzleHttp\Handler\EasyHandle $easy, \WPCOM_VIP\GuzzleHttp\Handler\CurlFactoryInterface $factory) : \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface
    {
     ...
     */
    private function getDefaultConf(\WPCOM_VIP\GuzzleHttp\Handler\EasyHandle $easy) : array
    {
    ...
    private function applyMethod(\WPCOM_VIP\GuzzleHttp\Handler\EasyHandle $easy, array &$conf) : void
    {
    ...
    private function applyBody(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options, array &$conf) : void
    {
    ...
    private function applyHeaders(\WPCOM_VIP\GuzzleHttp\Handler\EasyHandle $easy, array &$conf) : void
    {
    ...
    private function applyHandlerOptions(\WPCOM_VIP\GuzzleHttp\Handler\EasyHandle $easy, array &$conf) : void
    {
        ...
        if (isset($options['proxy'])) {
            if (!\is_array($options['proxy'])) {
                ...
                if (isset($options['proxy'][$scheme])) {
                    ...
                    if (!isset($options['proxy']['no']) || !\WPCOM_VIP\GuzzleHttp\Utils::isHostInNoProxy($host, $options['proxy']['no'])) {
        ...
        if (!empty($options['debug'])) {
            $conf[\CURLOPT_STDERR] = \WPCOM_VIP\GuzzleHttp\Utils::debugResource($options['debug']);
     ...
     * @param callable(RequestInterface, array): PromiseInterface $handler
     */
    private static function retryFailedRewind(callable $handler, \WPCOM_VIP\GuzzleHttp\Handler\EasyHandle $easy, array $ctx) : \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface
    {
        try {
    ...
    private function createHeaderFn(\WPCOM_VIP\GuzzleHttp\Handler\EasyHandle $easy) : callable
    {
