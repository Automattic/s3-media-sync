...
final class Middleware
{
    ...
    public static function cookies() : callable
    {
        return static function (callable $handler) : callable {
            return static function ($request, array $options) use($handler) {
                ...
                } elseif (!$options['cookies'] instanceof \WPCOM_VIP\GuzzleHttp\Cookie\CookieJarInterface) {
                    throw new \InvalidArgumentException('WPCOM_VIP\\cookies must be an instance of GuzzleHttp\\Cookie\\CookieJarInterface');
                }
                $cookieJar = $options['cookies'];
                $request = $cookieJar->withCookieHeader($request);
                return $handler($request, $options)->then(static function (\WPCOM_VIP\Psr\Http\Message\ResponseInterface $response) use($cookieJar, $request) : ResponseInterface {
    ...
    public static function history(&$container) : callable
    {
        ...
        return static function (callable $handler) use(&$container) : callable {
            return static function (\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options) use($handler, &$container) {
    ...
    public static function tap(callable $before = null, callable $after = null) : callable
    {
        return static function (callable $handler) use($before, $after) : callable {
            return static function (\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options) use($handler, $before, $after) {
                if ($before) {
    ...
    public static function redirect() : callable
    {
        return static function (callable $handler) : RedirectMiddleware {
            return new \WPCOM_VIP\GuzzleHttp\RedirectMiddleware($handler);
        };
    }
    /**
    ...
    public static function retry(callable $decider, callable $delay = null) : callable
    {
        return static function (callable $handler) use($decider, $delay) : RetryMiddleware {
            return new \WPCOM_VIP\GuzzleHttp\RetryMiddleware($decider, $handler, $delay);
        };
    }
    /**
    ...
    public static function prepareBody() : callable
    {
        return static function (callable $handler) : PrepareBodyMiddleware {
            return new \WPCOM_VIP\GuzzleHttp\PrepareBodyMiddleware($handler);
        };
    }
    /**
    ...
    public static function mapRequest(callable $fn) : callable
    {
        return static function (callable $handler) use($fn) : callable {
            return static function (\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options) use($handler, $fn) {
