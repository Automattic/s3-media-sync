...
class Client implements \WPCOM_VIP\GuzzleHttp\ClientInterface, \WPCOM_VIP\Psr\Http\Client\ClientInterface
{
    ...
    public function __construct(array $config = [])
    {
        if (!isset($config['handler'])) {
            $config['handler'] = \WPCOM_VIP\GuzzleHttp\HandlerStack::create();
        } elseif (!\is_callable($config['handler'])) {
            throw new \WPCOM_VIP\GuzzleHttp\Exception\InvalidArgumentException('handler must be a callable');
        }
    ...
    {
        if (\count($args) < 1) {
            throw new \WPCOM_VIP\GuzzleHttp\Exception\InvalidArgumentException('Magic request methods require a URI and optional options array');
        }
        $uri = $args[0];
        $opts = $args[1] ?? [];
    ...
    {
        $options[\WPCOM_VIP\GuzzleHttp\RequestOptions::SYNCHRONOUS] = \true;
    ...
    {
        $options[\WPCOM_VIP\GuzzleHttp\RequestOptions::SYNCHRONOUS] = \true;
        $options[\WPCOM_VIP\GuzzleHttp\RequestOptions::ALLOW_REDIRECTS] = \false;
        $options[\WPCOM_VIP\GuzzleHttp\RequestOptions::HTTP_ERRORS] = \false;
        return $this->sendAsync($request, $options)->wait();
    }
    /**
    ...
    public function requestAsync(string $method, $uri = '', array $options = []) : \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        $request = new \WPCOM_VIP\GuzzleHttp\Psr7\Request($method, $uri, $headers, $body, $version);
    ...
    {
        $defaults = ['allow_redirects' => \WPCOM_VIP\GuzzleHttp\RedirectMiddleware::$defaultSettings, 'http_errors' => \true, 'decode_content' => \true, 'verify' => \true, 'cookies' => \false, 'idn_conversion' => \false];
        // Use the standard Linux HTTP_PROXY and HTTPS_PROXY if set.
        // We can only trust the HTTP_PROXY environment variable in a CLI
        // process due to the fact that PHP has no reliable mechanism to
        // get environment variables that start with "HTTP_".
        if (\PHP_SAPI === 'cli' && ($proxy = \WPCOM_VIP\GuzzleHttp\Utils::getenv('HTTP_PROXY'))) {
            $defaults['proxy']['http'] = $proxy;
        }
        if ($proxy = \WPCOM_VIP\GuzzleHttp\Utils::getenv('HTTPS_PROXY')) {
            $defaults['proxy']['https'] = $proxy;
        }
        if ($noProxy = \WPCOM_VIP\GuzzleHttp\Utils::getenv('NO_PROXY')) {
            $cleanedNoProxy = \str_replace(' ', '', $noProxy);
            $defaults['proxy']['no'] = \explode(',', $cleanedNoProxy);
        }
        $this->config = $config + $defaults;
        if (!empty($config['cookies']) && $config['cookies'] === \true) {
            $this->config['cookies'] = new \WPCOM_VIP\GuzzleHttp\Cookie\CookieJar();
        }
        // Add the default user-agent header.
        if (!isset($this->config['headers'])) {
            $this->config['headers'] = ['User-Agent' => \WPCOM_VIP\GuzzleHttp\Utils::defaultUserAgent()];
        } else {
    ...
    private function prepareDefaults(array $options) : array
    {
        ...
        if (\array_key_exists('headers', $options)) {
            ...
            if ($options['headers'] === null) {
                ...
                throw new \WPCOM_VIP\GuzzleHttp\Exception\InvalidArgumentException('headers must be an array');
            }
        }
     ...
     */
    private function transfer(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options) : \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface
    {
     ...
     */
    private function applyOptions(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array &$options) : \WPCOM_VIP\Psr\Http\Message\RequestInterface
    {
        $modify = ['set_headers' => []];
        ...
        if (isset($options['form_params'])) {
            if (isset($options['multipart'])) {
                throw new \WPCOM_VIP\GuzzleHttp\Exception\InvalidArgumentException('You cannot use ' . 'form_params and multipart at the same time. Use the ' . 'form_params option if you want to send application/' . 'x-www-form-urlencoded requests, and the multipart ' . 'option to send multipart/form-data requests.');
            }
        ...
        if (isset($options['multipart'])) {
            $options['body'] = new \WPCOM_VIP\GuzzleHttp\Psr7\MultipartStream($options['multipart']);
     ...
     */
    private function invalidBody() : \WPCOM_VIP\GuzzleHttp\Exception\InvalidArgumentException
    {
        return new \WPCOM_VIP\GuzzleHttp\Exception\InvalidArgumentException('Passing in the "body" request ' . 'option as an array to send a request is not supported. ' . 'Please use the "form_params" request option to send a ' . 'application/x-www-form-urlencoded request, or the "multipart" ' . 'request option to send a multipart/form-data request.');
    }
}
