...
use WPCOM_VIP\GuzzleHttp\Exception\BadResponseException;
use WPCOM_VIP\GuzzleHttp\Exception\TooManyRedirectsException;
...
class RedirectMiddleware
{
    ...
    public function __invoke(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options) : \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface
    {
        ...
        return $fn($request, $options)->then(function (\WPCOM_VIP\Psr\Http\Message\ResponseInterface $response) use($request, $options) {
     ...
     */
    public function checkRedirect(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, array $options, \WPCOM_VIP\Psr\Http\Message\ResponseInterface $response)
    {
     ...
     */
    private function withTracking(\WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface $promise, string $uri, int $statusCode) : \WPCOM_VIP\GuzzleHttp\Promise\PromiseInterface
    {
        return $promise->then(static function (\WPCOM_VIP\Psr\Http\Message\ResponseInterface $response) use($uri, $statusCode) {
     ...
     */
    private function redirectUri(\WPCOM_VIP\Psr\Http\Message\RequestInterface $request, \WPCOM_VIP\Psr\Http\Message\ResponseInterface $response, array $protocols) : \WPCOM_VIP\Psr\Http\Message\UriInterface
    {
        $location = \WPCOM_VIP\GuzzleHttp\Psr7\UriResolver::resolve($request->getUri(), new \WPCOM_VIP\GuzzleHttp\Psr7\Uri($response->getHeaderLine('Location')));
        // Ensure that the redirect URI is allowed based on the protocols.
        if (!\in_array($location->getScheme(), $protocols)) {
            throw new \WPCOM_VIP\GuzzleHttp\Exception\BadResponseException(\sprintf('Redirect URI, %s, does not use one of the allowed redirect protocols: %s', $location, \implode(', ', $protocols)), $request, $response);
        }
        return $location;
    }
}
