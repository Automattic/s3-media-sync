...
class TreeInterpreter
{
    ...
    {
        $this->fnDispatcher = $fnDispatcher ?: \WPCOM_VIP\JmesPath\FnDispatcher::getInstance();
    }
    /**
    ...
    private function dispatch(array $node, $value)
    {
        ...
        switch ($node['type']) {
            ...
            case 'index':
                if (!\WPCOM_VIP\JmesPath\Utils::isArray($value)) {
                    return null;
                }
            ...
            case 'projection':
                ...
                switch ($node['from']) {
                    case 'object':
                        if (!\WPCOM_VIP\JmesPath\Utils::isObject($left)) {
                            return null;
                        }
                        break;
                    case 'array':
                        if (!\WPCOM_VIP\JmesPath\Utils::isArray($left)) {
                            return null;
                        }
                        break;
                    default:
            ...
            case 'or':
                ...
                return \WPCOM_VIP\JmesPath\Utils::isTruthy($result) ? $result : $this->dispatch($node['children'][1], $value);
            case 'and':
                $result = $this->dispatch($node['children'][0], $value);
                return \WPCOM_VIP\JmesPath\Utils::isTruthy($result) ? $this->dispatch($node['children'][1], $value) : $result;
            case 'not':
                return !\WPCOM_VIP\JmesPath\Utils::isTruthy($this->dispatch($node['children'][0], $value));
            case 'pipe':
                return $this->dispatch($node['children'][1], $this->dispatch($node['children'][0], $value));
            ...
            case 'comparator':
                ...
                if ($node['value'] == '==') {
                    return \WPCOM_VIP\JmesPath\Utils::isEqual($left, $right);
                } elseif ($node['value'] == '!=') {
                    return !\WPCOM_VIP\JmesPath\Utils::isEqual($left, $right);
                } else {
            ...
            case 'condition':
                return \WPCOM_VIP\JmesPath\Utils::isTruthy($this->dispatch($node['children'][0], $value)) ? $this->dispatch($node['children'][1], $value) : null;
            case 'function':
                $args = [];
            ...
            case 'slice':
                return \is_string($value) || \WPCOM_VIP\JmesPath\Utils::isArray($value) ? \WPCOM_VIP\JmesPath\Utils::slice($value, $node['value'][0], $node['value'][1], $node['value'][2]) : null;
            case 'expref':
